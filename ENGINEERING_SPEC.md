...
### 0-3-8. 사용자 개인정보·데이터는 절대 섞이거나 노출되면 안 됨 (2026-08-16 추가)

> **한 사용자의 로그인 세션·보유종목·입력값이 다른 사용자에게 보이거나, 두 사용자의 데이터가
> 서버 안에서 섞이는 일은 어떤 경우에도 있어서는 안 됩니다.** 지금까지 "내 성적표" 설계·
> #106 공개 전환 전 보안 점검에서 실질적으로 지켜온 원칙인데, 문서에 명문화된 적이 없어
> 이번에 §0-3 목록에 정식으로 올립니다.
...
- **애플리케이션 계층**: 로그인한 사용자의 데이터(Supabase 클라이언트, 보유종목, 세션 토큰
  ...
  앞으로의 NiceGUI 같은 상시 서버 프레임워크 전부 포함)에서는 이 실수 하나로 A의 데이터가
  B의 화면에 뜨는 사고가 날 수 있습니다. 읽기 전용 **시세 데이터**(`data/*.json` — 모든
  사용자에게 동일하고 개인정보가 아님)를 전역에 캐시하는 것과는 **반드시 구분**합니다.
- **검증 방법**: 로그인이 필요한 화면을 새로 만들거나 세션 처리 방식을 바꿀 때는, **서로 다른
  브라우저 세션(시크릿창 등)으로 동시 로그인해서 서로의 데이터가 절대 안 보이는지** 확인하는
  것을 완료 조건에 포함합니다.
