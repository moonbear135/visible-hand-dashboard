...
- **지금 이 프로젝트가 네트워크로 접속하는 대상과 각각의 매너 장치** (예시일 뿐입니다 — 새
  ...
  - **네이버 신 증권 API `stock.naver.com`**(JSON, 2026-09-07 추가 — 구 서비스 9/10 종료 대비)
    ...
    **요청 상한 460건**(목록 26 + 상세 200 + 위즈리포트 200 — 2026-09-08 오너 결정으로
    "좁게, 매일 전부"로 전환. 시총 상위 200종목을 매일 같은 대상으로 봅니다. 하루 약 426요청·18분,
    현행 수집기가 이미 하는 약 1,040요청의 41% 수준) + 연속 실패 5회 **서킷 브레이커** +
    **403/429 즉시 중단**(재시도·우회 없음). 목록 `pageSize` 는 화면이 실제 쓰는 20 을
    그대로 쓰고 **한도를 찾겠다고 큰 값을 시험하지 않습니다.** 상세를 전 종목(520개)이
    아니라 **표본 20종목**만 받는 것도 같은 이유입니다(관찰이 목적).
