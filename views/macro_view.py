...
def render_macro_page():
    ...
    if 'details' in locals() and len(details) >= 3:
        ...
        indicator_warnings = {
            ...
            "KOSPI_5D_Return": "코스피 지수의 5일 단기 낙폭 모멘텀이 심각하게 훼손되었습니다. 시장 참여자들의 투자 심리가 얼어붙어 작은 악재에도 투매가 쏟아질 수 있는 취약한 구간이므로 바닥이 확인될 때까지 신중해야 합니다."
        }
    ...
    with st.expander("📝 공식 알고리즘 거버넌스 및 5W1H 변경 이력 (Audit Trail)"):
        ...
        tab1, tab2, tab3 = st.tabs(["📄 v1.3.0 (최근 변경)", "📄 v1.2.0 (더미 제거)", "📄 v1.0.0 (최초 배포)"])
        
        with tab1:
            st.markdown(
                """
                #### 🏷️ [v1.3.0] - 2026년 08월 03일 (지표 추가 및 모델 가중치 정규화)
                * **언제 (When)**: 2026년 08월 03일 오후 시스템 패치 적용
                * **누가 (Who)**: **보이는 손 AI 분석팀**
                * **어디를 (Where)**: app.py / scrape_daily.py / views/macro_view.py
                * **무엇을 (What)**: 신규 변동성 지표(KOSPI 5일 낙폭 모멘텀) 편입에 따른 14개 지표 가중치 합산 100% 비율 정규화(Normalization) 적용
                * **왜 (Why)**: 백엔드 수집기(scrape_daily.py)에서 신규 지표가 추가되어 가중치 총합이 102%로 초과 산정되는 오류가 발생하였고, 이로 인해 프론트엔드와 수집 데이터 간의 불일치가 발견되어 이를 교정하기 위함
                * **어떻게 (How)**: 전체 가중치 합산이 정확히 100%가 되도록 기존 가중치 비율을 유지한 채 비례 축소 연산(`기존 가중치 * 100 / 102`)을 일괄 적용하여 시스템 간 정합성 일치 완료
                """
            )
        with tab2:
            st.markdown(
