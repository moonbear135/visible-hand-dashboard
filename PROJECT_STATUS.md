...
- **2026-08-09** — 종목별 시계열 이력 신설(#64) + UI 폴리시 6건(#65: 국기 이모지 깨짐, 툴팁 오버플로우, 안내박스 흐림 2건, 코스피 지수 가독성, 신선도 경고 관리자 전용화, 미국 화면 학습용 안내 박스).
...
- **🔴 다음 작업 — 매크로 14개 위험 지표 재설계.** "추정 프록시" 라벨링만으로는 부족 — 후행지표(장마감 후 확정 데이터)로 실제 연결 가능한 것만 남기고, 안 되는 지표는 화면에서 제거 검토. 조사된 무료 출처: KRX Open API(옵션 미결제약정), pykrx(VKOSPI·공매도, 단 공매도는 T+2 지연), 한국은행 ECOS(외환 스왑). ELS 낙인위험은 매칭되는 공개 데이터가 없을 가능성 높음. (매크로 화면 자체가 비공개라 급하지 않음)
