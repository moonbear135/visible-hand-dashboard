...
def create_duel_accounts_for_user(service_client, user_id, *, anchor_date=None):
    """
    (배치/서버 전용) 사용자 1명에게 계좌 3개(M1/M3/M6)와 시드 원장 3행을 만듭니다.
    ...
    에게 주지 않습니다). 즉 **시드 지급은 사용자 세션으로는 물리적으로 불가능**하고,
    계좌만 만들고 시드가 안 들어가면 "돈 없는 계좌"라는 반쪽 상태가 남습니다.
    그래서 옵트인 전체를 service_role 경로 하나로 묶습니다 — 이게 스키마의 권한 배치와
    유일하게 아귀가 맞는 구현입니다. (이 판단은 리뷰 대상입니다: 오너가 "옵트인은 앱이
    직접"을 원한다면 계좌 insert 만 앱이 하고 시드는 배치가 잇는 구조가 되는데, 그러면 그
    사이에 배치가 실패하는 창이 생깁니다.)
...
USER_WRITE_FUNCTIONS = ("save_order", "edit_order", "cancel_order", "save_consent")
