...
#
# ⚠️ 2026-08-18 오너 결정 — v2 OCR 5단계 실기기 검증이 사실상 마무리 단계라 100 → 15로
#    낮춥니다("아직 테스트를 더 해볼 수도 있다"는 이유로 원래 상시값 10보다는 약간
#    여유를 둠). Gemini 3.5 Flash-Lite 자체는 요청당 비용이 매우 작지만, 이 숫자는
#    "한 사람이 하루 몇 번 유료 API를 부를 수 있는가"의 유일한 상한선이므로 검증이 완전히
#    끝나면 10 등 상시 값으로 다시 낮출지 오너가 정할 것.
