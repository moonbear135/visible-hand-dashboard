...
def find_ticker_by_name(market, name, indexes):
    ...
    # 정확히 맞추고 입력했는데도 못 찾자, 사용자가 "통화(원화/달러)가 잘못돼서 그런가?"라고
    # 오해했습니다. 미국 쪽은 KR 처럼 전체 상장종목 보조 목록(broad_index)이 없어(아직
    # 미국판이 없음, resolve_stock_query 문서 참고) 이 함수의 문구가 그대로 최종 실패
    ...
    return None, None, (
        ...
        "밖이거나 이름이 정확히 일치하지 않아서일 수 있습니다. 정확한 종목코드/티커를 "
        "아신다면 이름 대신 코드로 입력해 주세요."
    )
...
#
# ⚠️ 2026-08-18 임시 상향 (10 → 100) — v2 OCR 5단계 실기기 재검증 중 배포 지연 문제로
#    오늘 하루 한도를 소진해 검증이 막혔습니다. 오너 결정으로 검증이 끝날 때까지 한시적으로
#    올려둡니다. Gemini 3.5 Flash-Lite는 요청당 비용이 매우 작아 이 정도 상향은 비용 부담이
#    거의 없습니다(실측: 요청당 입력 ~1000토큰·출력 ~50토큰 수준, Google AI Studio 사용량
#    대시보드로 오너가 직접 확인). **5단계 실검증이 끝나면 10으로 되돌리거나 오너가 정하는
#    상시 값으로 조정할 것** — 다음 세션은 이 주석을 지우지 말고 처리 여부를 먼저 확인하세요.
DAILY_OCR_UPLOAD_LIMIT = 100
