...
#
# ⚠️ 2026-08-18 오너 결정 — 15로 낮췄다가, 같은 날 모바일 실기기 검증 중 한도초과
#    오류를 만나 200으로 다시 올림("아직 테스트를 더 해볼 수도 있다"). Gemini 3.5
#    Flash-Lite 자체는 요청당 비용이 매우 작지만, 이 숫자는 "한 사람이 하루 몇 번 유료
#    API를 부를 수 있는가"의 유일한 상한선이므로 검증이 완전히 끝나면 10 등 상시 값으로
#    다시 낮출지 오너가 정할 것.
DAILY_OCR_UPLOAD_LIMIT = 200
