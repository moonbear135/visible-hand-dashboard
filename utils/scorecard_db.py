...
#
# ⚠️ 오너 결정 이력 — 100(최초) → 15(2026-08-18, 검증 마무리 단계) → 200(같은 날, 모바일
#    실기기 검증 중 한도초과 오류를 만나 "아직 테스트를 더 해볼 수도 있다"며 임시로 다시 올림)
#    → **15(2026-08-18, v2 실검증·공개 전환이 전부 끝나 상시 값으로 확정)**. Gemini 3.5
#    Flash-Lite 자체는 요청당 비용이 매우 작지만, 이 숫자는 "한 사람이 하루 몇 번 유료 API를
#    부를 수 있는가"의 유일한 상한선이라 검증용으로 풀어뒀던 200을 상시 서비스 값으로 되돌림.
DAILY_OCR_UPLOAD_LIMIT = 15
