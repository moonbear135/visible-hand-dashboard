...
#     ⚠️ (#204, 2026-09-07) 두 발행 워크플로우가 이제 **크롤링 완료 `workflow_run`** 으로도 돕니다
#        (원화: `scrape.yml` 완료 직후 = 거래일 D 저녁 17:2x~18:0x / 달러: 미국 종목·벤치마크 완료
#        직후 = D+1 아침 08:2x~08:4x). 그러면 결투 기준값 파일이 **아직 D-1 것일 수도, 결투가 몇 초
#        먼저 커밋해 D 것일 수도** 있습니다(결투 배치도 같은 이벤트로 동시에 깨어남). 위 규칙은 두
#        경우를 모두 이미 다룹니다 — 다른 날이면 결투 기준선, 같은 날이면 이력 CSV 직전 행 — 판정
#        함수는 같고 결과도 같습니다. 코드를 바꾸지 않았고, 워크플로우의 게이트(`crawl_ready_gate.py`)가
#        "오늘 자 수집이 실제로 끝났을 때"만 이 배치를 깨웁니다. cron(07:35 / 11:35)은 안전망으로
#        남아 같은 값을 한 번 더 발행합니다(그날 발행분 통째로 갈아끼우기 — 멱등).
#
