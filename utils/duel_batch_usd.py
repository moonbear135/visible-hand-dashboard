...
9:30am ET 전)**과 겹칩니다. 그래서 "X일에 접수된 주문의 체결 거래일(`resolve_fill_trading_day`
의 결과)"은 X 이후 첫 확정 거래일, 즉 보통 X+1일이 되고, 그 X+1일의 확정 종가는 다시 한국
날짜로 X+2일 아침에야 들어옵니다 — **배치는 항상 "실행되는 한국 날짜의 하루 전"에 해당하는
거래일을 처리해야** 합니다(`run_duel_daily_batch_us.py`가 `--target-date`를 생략하면 "어제"를
기본값으로 쓰는 이유입니다).
