...
# -----------------------------------------------------------------------------
# 📅 '일간'에서 거래일이 아닌 날을 골랐을 때 — 가장 최근 거래일로 대체 (2026-08-16 #117)
# -----------------------------------------------------------------------------
#  오너 원문: "일간 보고서를 보면 주말 공휴일에는 자료가 안나오게 되있는데 이건 가장 마지막
#  날을 기준으로 그냥 계속 보여줘도 되는거 아냐? 굳이 이렇게 비워둘 필요는 없을거 같은데".
#
#  '일간' 기간의 창은 그 날 하루뿐이라(period_bounds), 토·일·공휴일을 고르면 그 창에 스냅샷이
#  하나도 없어 화면 전체가 "데이터 부족"만 남았습니다. 장이 안 열린 날은 **데이터가 없는 게
#  정상**이므로, 그 이전 가장 최근 거래일의 기록을 대신 보여 줍니다.
#
#  🔴 §0-1(지어내지 않기)을 지키는 방식 — 이 함수는 **아무 값도 만들지 않습니다.**
#     · 실제로 저장돼 있는 스냅샷의 **날짜만** 고릅니다(값 보간·역산 없음).
#     · 대체가 일어났는지를 두 번째 반환값으로 **반드시 함께** 돌려주고, 화면은 그것을 받아
#       "선택한 날 ≠ 보여주는 날"을 눈에 띄게 밝힙니다. 조용히 바꿔치기하면 그건 거짓말입니다.
#     · 그 이전에도 스냅샷이 하나도 없으면 대체하지 않습니다(= 기존 "데이터 부족" 그대로).
#
#  ⚠️ **'일간'에서만** 동작합니다. 주간~연간은 창 안의 실제 스냅샷들을 모으는 방식이라 이
#     문제가 없고(주중 하루라도 기록이 있으면 리포트가 나옵니다), 그 기간들까지 대체하면
#     "2026년 9월 리포트"에 8월 데이터가 실리는 전혀 다른 이야기가 됩니다.
# -----------------------------------------------------------------------------
def resolve_display_date(snapshots, period, ref_date):
    """
    (표시용 날짜, 대체 여부)를 돌려줍니다.

    · 일간이 아니거나 / 기준일에 스냅샷이 실제로 있으면  → (기준일, False)  ← 기존 동작 그대로
    · 일간이고 기준일에 스냅샷이 없으면                  → (기준일 이전 가장 최근 스냅샷 날짜, True)
    · 그 이전에도 스냅샷이 하나도 없으면                 → (기준일, False)  ← 대체하지 않음
    """
    day = to_date(ref_date)
    ...
    for row in snapshots or []:
        raw = (row or {}).get("snapshot_date")
        if raw is None or raw == "":
            continue
        dates.add(to_date(raw))
