...
-------------------------------------------------------------------------------
  ...
  · 5-6  최소 참가 인원(1명, 2026-09-03 500→1) → `MIN_PARTICIPANTS_FOR_PUBLICATION`
...
#: 🔴 최소 참가 인원(5-6). 이 인원을 못 채운 (통화 × 체급) 그룹은 아예 발행하지 않습니다.
#:
#:    2026-09-03 오너 확정 — **500 → 1.** 원래는 "3명짜리 구간에서 '1위 닉네임'은 사실상
#:    실명"이라는 익명성 논리로 500명이었습니다(2026-08-20 확정). 오너가 이 문턱을 없애기로
#:    했습니다: "실사용자가 적어 500명을 기다리면 기능이 시작조차 안 된다. 어차피 순위표에
#:    올라가는 건 본인이 명시적으로 동의한 사람뿐(6개 항목 전부 동의 + 최종확인)이라 인원이
#:    적다고 굳이 숨길 필요는 없다." 즉 동의자가 1명이라도 있으면 그 그룹은 발행됩니다.
#:
#:    ⚠️ 이 상수와 `group_meets_minimum()` 은 지금 "내 성적표" 공개 순위표
#:       (`utils/scorecard_publish.py`)만 씁니다. 결투(`/duel`) 공개 순위표(Branch 2)는
#:       2026-08-23 에 폐기됐고 `/duel` 개인 가상계좌 기능은 이 값을 읽지 않습니다.
#:    ⚠️ 값을 1 로 두되 상수·판정 함수·미달 그룹 청소 로직은 **그대로 남깁니다** — 나중에
#:       문턱을 다시 올릴 때 이 숫자 하나만 바꾸면 되도록.
MIN_PARTICIPANTS_FOR_PUBLICATION = 1
...
#     `MIN_PARTICIPANTS_FOR_PUBLICATION`(지금 1, 원래 500) 과 아래 `LEADERBOARD_TOP_COUNT`
#     (500) 는 **완전히 다른 개념**입니다 — 하나는 "몇 명부터 그룹을 발행하는가"(5-6),
#     다른 하나는 "공개된 순위표에서 몇 등까지 보여줄 것인가"(5-7)입니다. 한쪽을 바꾼다고
#     다른 쪽이 따라 바뀌면 안 되므로 상수를 따로 둡니다(2026-09-03 에 5-6 만 500→1 로
#     바뀌었고 5-7 은 그대로인 것이 그 실례입니다).
