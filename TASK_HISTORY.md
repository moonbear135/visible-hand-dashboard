...
- GitHub Actions schedule 트리거 자체의 지연·누락(2026-08-27·28·31, scrape.yml·
  ...
  특정하거나 고칠 수 있는 부분이 아님(§0-1). ✅ 다만 2026-08-31 **#184**에서
  ...
  아니라 `data_sanity.py` 쪽 책임으로 범위를 명확히 분리함). 자동 재실행 API
  호출 자체의 실전 동작 확인(`test_suite.yml`을 임시 대상으로 한 실전 테스트)은
  아직 오너가 해야 할 몫으로 남아있음. #154 워치독은 여전히 "원인 제거"가 아니라
  "재발 시 놓치지 않고 알아채고, 가능하면 스스로 한 번 고쳐보기" 용도임을 오너도
  인지하고 있음.
