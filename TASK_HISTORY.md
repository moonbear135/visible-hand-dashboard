...
### #200 — [실전 발견] 스케줄 워치독의 "한국 주말이면 5개 통째로 건너뛰기" 블랭킷 게이트 제거 — 데이터 파일이 있는 대상은 토·일에도 검사, duel_daily.yml 만 계속 주말 스킵 (2026-09-05)

**경위.** 오너가 토요일(09-05) 11:55 KST 에 `watch_schedule_health.yml` 을 수동 실행(workflow_dispatch)해 보니 로그에
`scrape.yml / scrape_us.yml / indicator_kr.yml / scrape_report_snapshots.yml / duel_daily.yml — 주말이라 이번 검사에서 건너뜁니다.`
가 찍힘. 오너가 데이터 파일로 직접 확인한 바 미국 2개는 9/4(금) 미국 마감분을 9/5(토) 새벽(05:35 / 08:20 KST)에 정상 커밋했지만,
**만약 실패했다면 이 게이트 때문에 한국 주말 내내(월 18:00 KST 까지) 아무도 몰랐을** 구조였음. 오너 결론: "요일별로 대상을
쪼개지 말고, 한국이든 미국이든 토요일까지는 검사가 계속 돌게 두자".

**조사 결과(코드·주석·cron 실측, §0-1).**
- 게이트의 유래: 2026-08-28 신설 당시 주석 — "평일 전용 워크플로우는 주말엔 건너뛰고, 월요일엔 창을 76시간으로". 그때는
  conclusion 만 봤으므로, 자기 cron 이 월~금인 5개는 주말에 "최근 30시간 내 성공 실행 없음"이 **정상**이라 그대로 두면 매주
  토·일 거짓 경고 → 그걸 막으려던 게이트가 맞음(추측이 코드로 확인됨).
- 대상별 실제 cron: `scrape.yml` `5 7 * * 1-5`(16:05 KST) / `indicator_kr.yml` `0 8 * * 1-5`(17:00) / `duel_daily.yml` `10 8 * * 1-5`
  (17:10, + scrape.yml 완료 workflow_run) / `scrape_us.yml` `35 20|21 * * 1-5`(금 발동분 = **토 05:35 KST**) /
  `scrape_report_snapshots.yml` `20 23 * * 1-5`(금 발동분 = **토 08:20 KST**) — 5개 모두 요일 필드 `1-5`. daily 5개
  (`duel_daily_us.yml` `0 3 * * *` 포함)는 모두 `*`. `duel_daily_us.yml` 은 원래도 daily 라 스킵 대상이 아니었음(묶임 오류 없음).
- 데이터 판정 규칙을 주말 시각으로 실제 실행(워크플로우에서 뽑은 인라인 파이썬, `run_verdict`): kr — 토·일 검사의 기대 거래일 =
  금, 금 16:05 ✅ / 목 16:05 ❌ / 금 07:04(#195 모양) ❌. us — 토·일 = 금 세션, 금 16:35 ET ✅ / 목 ❌ / 토 05:10 ET 재실행분 ✅.
  daily — 토 검사 기대 실행일 = 토. **주말에 돌려도 오탐 없음** 확인. 단 하나 발견: kr 은 "날짜 == 기대 거래일" 등호 규칙이라,
  주말에 kr 대상이 미갱신으로 잡혀 자동 재실행이 걸리면 백필 없는 수집기가 **토요일 날짜**로 저장(내용은 금 종가) → 일요일 검사가
  그 파일을 "기대 거래일(금)의 데이터가 아님"으로 또 잡아 재실행 연쇄. 이건 등호를 부등호로 바꿔 해결(아래).

**결정 — 대상별 주말 처리.**

| 대상 | 자기 cron | 워치독이 보는 것 | 주말(토·일) 검사 | 이유 |
|---|---|---|---|---|
| `scrape.yml`, `indicator_kr.yml` | 월~금 | 실행 + 데이터 파일(kr) | **이제 검사함** | 판정이 "가장 최근 기대 거래일(금) 장마감 이후"라 오탐 없음. 미갱신이면 토·일 저녁 자동 재실행이 금 종가를 채움(주말엔 장중 아님) |
| `scrape_us.yml`, `scrape_report_snapshots.yml` | 월~금(ET) | 실행 + 데이터 파일(us) | **이제 검사함** | 금요일 마감분이 토 새벽 KST 에 오는 대상 — 바로 오너가 지적한 사각지대. 세션 규칙이라 주말 재실행분도 정상 판정 |
| `watch_dividend_disclosures.yml`, `watch_dividend_payment_events.yml` | 매일 | 실행 + 데이터 파일(daily@) | 원래부터 검사함(변경 없음) | — |
| `duel_daily.yml` | 월~금 | 실행 결과(conclusion)만 | **계속 건너뜀** | 결과가 Supabase 에만 남아 확인할 파일이 없고(#199), 주말엔 "최근 성공 실행 없음"이 정상 → 검사하면 매주 토·일 거짓 경고 + 헛된 재실행. 월요일 76h 창이 금요일치를 봄 |
| `duel_daily_us.yml`, `scorecard_publish_daily.yml`, `watch_data_sanity.yml` | 매일 | conclusion 만 | 원래부터 검사함(변경 없음) | — |

일요일도 검사하는 이유(오너는 "토요일까지는"이라 했지만): 판정 규칙상 일요일이라고 오탐이 생길 이유가 없고, 일요일에 봐야
"토요일 자동 재실행이 실제로 파일을 채웠는지"를 하루 더 빨리 확인함. 일요일만 막을 근거가 없어 막지 않음.

**구현 (`.github/workflows/watch_schedule_health.yml`).**
- 주말 스킵 조건을 `KIND=weekdays` → **`KIND=weekdays && DATA_FILE 없음`** 으로. 파일명이 아니라 필드 모양으로 판단하므로 나중에
  duel_daily.yml 에 데이터 파일을 붙이면 자동으로 주말 검사에 들어감. 스킵 로그 문구도 이유가 보이게 변경
  ("평일 전용 cron 이고 확인할 데이터 파일이 없어(실행 결과만 봄) 주말엔 '최근 실행 없음'이 정상이라 …").
- weekdays 창: 월 76h(종전) 외에 **일 54h**(금 16:05 발동분이 50h 전 + 4h 여유) 추가, 토는 30h(26h 전 — 평일 "어제치"와 같은 거리).
  `case "$DOW_KST"` 로 정리. 창이 넓어도 데이터 판정이 따로 있어 데이터 대상에는 해롭지 않음.
- kr 판정: `stamp_date != expected` → **`stamp_day < expected`**(❌ 문구 그대로) / `stamp_day == expected and 장마감 전`(❌ #195
  문구 그대로) / **`stamp_day > expected` → ✅ 새 문구**("… 장마감 뒤 비거래일·장 시작 전에 수집됨(그 거래일 종가)") / 나머지 ✅
  문구 그대로. "기대 거래일 장마감 이후에 수집됐다 = 그 종가를 담았다"이므로 부등호가 원래 의도에 더 정확. 부수 효과로 "금요일
  tick 이 월요일 새벽 07:04 에 늦게 발동한 파일"을 월요일 아침 수동 검사(기대 금)에서 오탐하던 것도 사라짐(월 18:00 정규 검사에선
  종전처럼 #195 모양으로 ❌).
- 헤더에 🔴 #200 문단(경위·근거·창·kr 완화), "확인 대상" 목록·"어떻게 확인하는가" 문단 갱신. 재실행·이슈·디스코드 단계는 불변.

**회귀 테스트 (`tests/test_watch_schedule_health_window.py` §8 신설, +12 → 100).** 기존 2건(`test_weekend_skips_weekday_only_targets_entirely`
→ `..._only_the_conclusion_only_weekday_target`, `test_weekend_midday_is_not_market_hours`)은 "주말이면 5개 스킵" 전제를 "duel_daily.yml
만 스킵, 9개 재실행"으로 갱신. 신규: 스킵 집합을 TARGETS 필드 모양에서 재계산해 `["duel_daily.yml"]` 과 대조 + weekdays/daily 표기가
각 대상 cron 요일 필드(`1-5`/`*`)와 일치(duel_daily_us.yml 이 daily 임을 포함) / 토 18:00·일 18:00·**토 11:55(오너 실측 순간)** 세
시각에 금요일 정규 발동분 + 정상 파일이면 재실행 0·6개 "갱신 확인"·건너뜀 1건·창 30/54 로그 / 토요일에 미국 2개가 목요일치면 ❌ +
재실행 2건(예전 게이트가 숨기던 바로 그 경우) / 미국 2개 실행 자체가 없으면 "실행 없음" ❌ / 한국 2개 목요일치·#195 모양 ❌ /
창 경계(일 50h 안·55h 밖, 토 26h 안·50h 밖) / kr 완화(토 18:10 파일: 일 ✅·월 08:00 ✅·월 18:00 ❌, 월 07:04 파일: 월 08:00 ✅·
월 18:00 ❌ 문구 불변) / kr 이 목요일치·장마감 전은 주말에도 ❌ 문구 불변 + 15:30 경계 / us 주말 판정 / 평일 동작 불변(⏭ 0건).
수정 전 워크플로우(HEAD)로 되돌려 §8 을 돌리면 **10개 실패** 확인.

**검증.** 관련: `pytest -q tests/test_watch_schedule_health_window.py` **100 passed**(88 + 12). 전체 `pytest -q --ignore=archive`
****2,227 passed / 73 skipped**(#199 의 2,215 + 신규 12, 회귀 0; `--collect-only` 2,300건과 일치. 실행 환경의 명령 시간 상한(3분) 때문에 46개 파일을 3묶음(1,346 / 781+73 skipped / 100)으로 나눠 돌렸고 합계가 일치)**.

**하지 않은 것(§0-1).** (a) duel_daily.yml 에 데이터 기준을 만들지 않음(#199 결론 그대로). (b) 공휴일(한국·미국) 판정은 여전히 없음
...
→ **#202(2026-09-06)에서 오너 승인으로 cron 을 11:35 KST(02:35 UTC)로 옮겨 미국 검사 활성화** (근거는 #202 참고).
→ **#203(2026-09-07)에서 원화·달러 워크플로우 분리** — 미국만 11:35, 원화는 07:35 로 복귀(원화가 미국을 기다리던 부작용 제거).
...
대기 조용한 스킵(삭제·삽입·holdings 조회 0건) + 스킵 날도 철회 청소 + 한 시장 skip 이면 전체 스킵(→ #203 에서 "담당 통화만 스킵"으로 변경) + abort 우선 / (d) 기준선
...
활성화는 오너 결정 → **#202 에서 승인·이동 완료**). (c) 이력 CSV 를 기준선으로 쓰는 것은 지시("결투 기준선 재사용")에서 벗어난 임의 판단 — 이유는 위 "조사 결과".
...
### #202 — [#201 후속, 오너 승인] 성적표 발행 배치 cron 을 07:30 KST → **11:35 KST** 로 이동 — 미국 벤치마크 수집 뒤로 옮겨 미국 쪽 신선도 검사 활성화 (2026-09-06)

**경위.** #201 이 붙인 신선도 검사는 미국 벤치마크(`data/us_index_history.json`, `scrape_report_snapshots.yml` cron 23:20 UTC =
08:20 KST)를 읽는데, 발행 배치 cron 이 그보다 이른 22:30 UTC(07:30 KST)라 미국 쪽은 "지수 원천이 낡음"으로 **매일 생략**됐음.
#201 은 "cron 을 옮기면 활성화된다 — 오너 결정 사항"으로 남겼고, 오너가 "이것까지 시작해 달라"고 명시적으로 승인.

**조사 결과 (§0-1 — 값을 정하기 전에 실측).**
- 실제 실행 기록(오너 컴퓨터에서 공개 Actions API `workflows/{file}/runs` 조회, 2026-09-06)과 저장소 커밋의
  `us_index_history.json` → `metadata.collected_at_kst`:

  | 날짜 | `scrape_report_snapshots.yml` 실제 시작(cron 08:20) | 벤치마크 커밋(`collected_at_kst`) | `scrape_us.yml` 실제 종료(cron 05:35) | 성적표 발행 실제 시작(cron 07:30) |
  |---|---|---|---|---|
  | 08-19~08-26 | 08:40~08:44 | 08:41~08:44 | — | 07:48~07:55 |
  | 08-27 / 28 / 29 | 13:32 / 15:57 / 13:11 | 13:33 / 15:58 / 13:11 | — / — / 12:47 | 12:27 / 15:13 / 12:48 |
  | 09-01 | 10:51 | 10:52 | 09:54 | 10:21 |
  | 09-02 / 03 / 04 / 05 | 10:04 / 10:10 / 10:03 / 10:04 | 10:05 / 10:11 / 10:04 / 10:04 | 08:36 / 08:39 / 08:38 / 08:20 | 09:24 / 09:30 / 09:16 / 09:16 |

- 읽는 법: GitHub 스케줄러 지연(`watch_schedule_health.yml` 머리말·`cloudflare_worker.js`, 2026-08-27 이후 매일 1.5~7시간)이
  **모든** cron 에 비슷하게 걸리지만 워크플로우마다 ±15~27분씩 다름. 종전 07:30 은 제때 돌았다면 미국 벤치마크는 물론 **미국
  종목 스냅샷(08:20~08:39 종료)조차** 어제 값으로 발행했을 시각이고, 지금까지 순서가 맞아 보인 것은 이 배치도 같은 지연을
  받아 09:07~10:21 에 돌았기 때문(운에 기댄 순서). 미국 벤치마크는 정작 그보다 더 늦은 10:04~10:52 에 들어와 검사가 매일 생략됨.
- 오너가 예시로 든 "08:20 + 10분"류의 빠듯한 값은 위 지연 편차(±27분)와 소요(~1분+커밋)만으로도 뒤집힐 수 있음 — 08-29 는
  두 워크플로우 지연 차가 27분이라 30분 여유로도 3분밖에 안 남았을 날. 이 저장소의 관례는 앞 단계 뒤 **30분 이상**
  (`duel_daily.yml` 머리말: 10분→30분으로 넓힌 전례, `duel_daily_us.yml`: "처음부터 넉넉하게" 4시간).

**결정 — cron `35 2 * * *` (02:35 UTC = **11:35 KST**, 매일).** 근거 다섯 가지(워크플로우 머리말 🕘 #202 문단에 같은 내용):
① 벤치마크 cron(08:20 KST)보다 **3시간 15분** 뒤. ② 지난 한 주의 **실제** 커밋 시각 중 가장 늦은 10:52 보다도 43분 뒤(30분 관례
충족 — 명목이 아니라 실측 기준으로). ③ 결투 USD 배치(`duel_daily_us.yml` cron 12:00 KST, 실제로는 16:18~18:39)보다 앞이라 §4-b 가
전제한 "미국 결투 기준선 = D-1 값"이 명목 시간표에서 유지되고, 지연으로 넘겨도 이력 CSV 대체 경로가 받음. ④ 워치독
`watch_schedule_health.yml`(18:00 KST — 감시 대상 전부의 예정 시각 뒤여야 하며 `test_watchdog_cron_runs_outside_market_hours_and_after_every_target`
가 고정)보다 앞. ⑤ 정각·정30분 회피 관례(`scrape.yml` 07:05, `scrape_us.yml` 20:35). KST = UTC+9 는 한국에 서머타임이 없어 연중
고정이므로 이 값은 계절과 무관(`scrape_us.yml` 처럼 두 줄이 필요 없음). 코스피 쪽은 영향 없음 — 11:35 에도 스냅샷은 어제
16:05 수집분이고 결투 KR 기준선도 어제 것이라 #201 의 KR 분석(같은 날 → 이력 CSV 대체)이 그대로.

**바꾼 것.**
- `.github/workflows/scorecard_publish_daily.yml`: cron `30 22 * * *` → `35 2 * * *`. 머리말 시간표에 `scrape_report_snapshots.yml`
  추가, 🕘 #202 문단(실측 표 요약·다섯 근거·남는 한계) 신설, #201 문단의 "07:30 이라 미국 검사 항상 생략 — 오너 결정 사항"을
  "✅ #202 로 활성화"로 갱신.
- `utils/scorecard_publish.py` §4-b 머리말: "07:30 KST" 서술을 "#201 당시 07:30 / #202 부터 11:35" 로, "미국 검사 항상 생략"을
  "#202 에서 활성화 + 12:00 을 넘겨 돌면 이력 CSV 경로" 로 갱신. 코드 변경 없음.
- `run_scorecard_publish_batch.py` 머리말: 미국 벤치마크 수집이 (신선도 검사 때문에) 사실상 앞 단계가 됐음을 한 줄 추가.
- `SCORECARD_PUBLIC_LEADERBOARD_WORK_ORDER.md` Phase A 표기(`30 22 * * *` = 07:30)에 이동 사실 병기. #201 항목의 "옮기지 않음"
  두 곳에 → #202 표시.
- `tests/test_scorecard_publish.py`: 신규 `test_publish_cron_runs_after_us_benchmark_with_repo_margin_and_before_watchdog` — 워크플로우
  파일에서 cron 을 **읽어**(값을 베끼지 않음) ① 벤치마크 cron 뒤 30분 이상 ② 결투 USD·워치독 cron 보다 앞 ③ 정각·정30분 아님
  ④ 매일(요일 필드 `*`) 을 고정. 나중에 누가 벤치마크 시각을 늦추면 이 테스트가 빨간불로 알려 줌.

**남는 한계(정직하게).** ⚠️ 이 cron 이동은 원화 발행까지 같이 늦추는 부작용이 있어 **#203 에서 원화·달러 워크플로우 분리로 되돌림**(달러만 11:35, 원화는 07:35). 08-27~29 처럼 벤치마크가 13~16시에야 들어오는 날은 여전히 미국 검사가 생략될 수 있음 — 그런 날엔 이
배치도 같은 지연을 받아 대체로 순서가 지켜지고, 안 지켜져도 결과는 "미국 검사 생략 + 요약에 사유"(종전과 같음)이지 틀린 판정이
아님. 근본 해결은 `duel_daily.yml` 처럼 `workflow_run`(Daily Report Snapshots 완료)에 묶는 것인데, 이 배치는 주말에도 돌아야 해서
cron 을 없앨 수 없고 트리거 추가는 별도 결정 사항으로 남김(이번 지시 범위는 cron 이동).

**🔎 조사 중 발견 — cron 과 무관한 #201 의 빈틈(코드 안 고침, 오너 결정 사항).** 저장소 실데이터로 `load_freshness_inputs()` 를
돌려 보니 결투 USD 배치가 **토·일 대상(target = 토·일)으로 남기는 기준값 파일은 지수가 없음**(`index_keys: []`, 종목 50개만 —
`data/duel_freshness_probe_previous_usd.json` 커밋 이력 08-30/08-31/09-06 분 전부 그 모양). #201 의 규칙은 "결투 기준선 날짜가 스냅샷
거래일과 **다르면** 그것을 쓴다"라서, 월요일(기준선 = 토요일 target, 스냅샷 = 금요일 세션)과 화요일(기준선 = 일요일 target, 스냅샷 =
월요일 세션)엔 지수 없는 결투 기준선을 집어 `no_baseline`(검사 생략)이 됨 — **cron 을 옮겨도 매주 월·화 이틀은 미국 검사가 생략**되고,
수·목·금·토·일 5일만 실제로 판정함(2026-09-06 일요일 데이터로 확인: US → `no_baseline` "전일 기준값에 지수가 없어"). 고치려면
`load_freshness_inputs()` 에서 "결투 기준선에 지수가 없으면 이력 CSV 직전 행으로 대체"를 추가하면 되는데(같은 날 규칙과 같은 대체
경로), #201 이 "기준선에 지수 없음 → 생략"을 테스트로 고정해 둔 동작이라 이번 지시 범위(cron 이동) 밖으로 두고 오너 결정으로 남김.
→ **#203(2026-09-07)에서 닫음** — `load_freshness_inputs()` 가 지수 없는 결투 기준선을 "없는 것"으로 보고 이력 CSV 직전 행으로 대체.

**검증.** `pytest -q tests/test_scorecard_publish.py` **192 passed**(#201 의 191 + 신규 1), `tests/test_watch_schedule_health_window.py` +
`tests/test_report.py` **123 passed**(워치독 "대상 모두의 예정 시각 뒤" 검사가 새 11:35 를 그대로 통과). 실제 GitHub 실행은 오프라인
검증 밖 — 위 빈틈 때문에(→ #203 에서 닫힘) 미국 검사의 첫 실제 판정은 **09-09(수) 11:35 KST** 실행 요약의 "🩺 … US: 검사함 → ok — 기준선 2026-09-07
(duel_probe)" 줄에서 확인(09-07 월·09-08 화는 `no_baseline` 생략이 정상). ⚠️ 09-07 은 미국 Labor Day 휴장이라 결투가 그날 대상으로
남기는 기준선에도 지수가 없을 가능성이 커서, 첫 실제 판정이 **09-10(목)** 로 하루 더 밀릴 수 있음(휴장일 기준선 모양은 §0-1 상 실측
전 — 그날 요약으로 확인). 워치독(18:00) 이슈 없음도 함께 확인. GitHub push 권한이 없는 세션이라 **로컬 커밋까지만** 했음 — 오너가 push.

### #203 — [#202 후속, 오너 결정] 성적표 발행을 결투처럼 **원화·달러 워크플로우 두 개로 분리** — 원화 07:35 KST 로 복귀, 달러만 11:35 KST + 🔴 통화 간 침범 금지 정합성 + #202 의 월·화 `no_baseline` 빈틈 봉합 (2026-09-07)

**경위.** #202 가 미국 신선도 검사를 살리려고 발행 배치 cron 을 07:30 → 11:35 KST 로 옮겼는데, "내 성적표" 발행은 원화·달러를
한 배치(`run_publish_batch()`)에서 같이 발행하므로 **전날 16:05 에 이미 확정된 코스피 성적표까지 미국 지수를 기다리느라 네 시간
늦게** 나가게 됐음(오너 지적: "한국 데이터는 기다릴 이유가 없다"). 오너 결정: 결투(`duel_daily.yml` / `duel_daily_us.yml`)처럼
한국·미국 발행을 분리하되, **어느 쪽이든 필요 이상으로 늦추지 말 것**(안정적으로 검사를 통과할 최소 여유만).

**시작 전 확인.** `git pull --rebase origin main` → Already up to date. 로컬 HEAD = origin/main = `d7af0dd`(#202 커밋). 지시문의
"직전 커밋 `ad67fc3`" 과 해시가 다르지만 내용(#202 cron 이동)은 그 커밋이 맞음 — 해시 표기 차이만 보고.

**바꾼 것.**
- `utils/scorecard_publish.py` — `run_publish_batch(..., currencies=None)` 신설(기본 전체 = 하위호환). 🔴 통화 간 침범 금지:
  ① 5단계 가지치기 `all_possible_groups(currencies)` — 담당 통화의 9개 그룹만. 18개 전부를 보면 한국 배치가 "오늘 달러 그룹을
  발행 안 했다"고 오판해 달러 과거 행 전부 삭제(§0-1 위반). ② 6단계 당일 삭제 `delete_published_rows_for_date(currencies=)` —
  `currency in (...)` 로 좁힘(미국 배치가 그날 아침 원화 행을 지우지 않게). ③ 3단계 체급 배정·4단계 조립(`build_publish_rows(currencies=)`)
  도 담당 통화만. ④ 신선도: `load_freshness_inputs(markets=)` 로 담당 시장만 읽고, 손으로 넣은 입력도 범위 밖 시장은 판정 전에
  버림. ⑤ 정책 변경 — #201 의 "시장 하나라도 skip 이면 전체 스킵"(18개 가지치기 시절의 방어)을 **"그 시장을 담당하는 통화만
  스킵"** 으로(두 통화 한 번에 도는 실행에서도 통화별 판정; `abort` 는 여전히 전체 중단). ⑥ H-3 "거래일 확인"도 담당 시장 기준.
  요약 dict 에 `requested_currencies` / `currencies` / `skipped_currencies` 추가, `publish_skipped` 는 요청 통화가 **전부** 건너뛴 때만.
  새 도우미 `normalize_currencies()` / `markets_for_currencies()` / `market_for_currency()`(통화↔시장은 `scorecard_db.CURRENCY_BY_MARKET`
  단일 출처). 철회 청소(0단계)는 통화와 무관한 의무라 **두 배치 모두** 실행(멱등 — 두 번째는 조회 1개로 끝남; "하루도 거르면 안 되는
  일"에 기회가 하루 두 번). 머리말 ⑦·§4-b 머리말·5단계 주석 갱신.
- `utils/scorecard_publish_db.py::delete_published_rows_for_date(…, currencies=None)` — `.in_("currency", …)` 필터. 빈 목록은 거절
  (빈 목록을 "전부"로 읽지 않음), 모르는 통화는 `_require_currency()` 가 거절.
- `run_scorecard_publish_batch.py` — `--currency {KRW,USD}`(반복 가능, 생략 = 전체). 결투는 모듈부터 달라 **파일**을 나눴지만 이
  배치는 `currency` 가 한 모듈의 축이라 파일을 나누면 같은 코드가 두 벌 생기므로(§0-3-10) **인자**로 가름 — 머리말 💱 문단에 근거.
  한 통화만 건너뛴 날도 `::warning title=성적표 {통화} 발행 건너뜀::` 출력.
- `.github/workflows/scorecard_publish_daily.yml`(원화) — `ARGS="--currency KRW"`, cron `35 2 * * *` → **`35 22 * * *`(07:35 KST)**.
  머리말에 💱 #203 분리 배경·정합성 조건·🕘 #203 시각 근거. `.github/workflows/scorecard_publish_daily_us.yml`(달러, 신규) —
  `ARGS="--currency USD"`, cron **`35 2 * * *`(11:35 KST)**, concurrency `scorecard-publish-batch-usd`(원화와 다른 그룹 — 통화별 행만
  만지므로 겹쳐도 안전, 같은 트랙끼리만 직렬). `permissions: contents: read` 동일.
- `.github/workflows/watch_schedule_health.yml` — TARGETS 에 `scorecard_publish_daily_us.yml|daily` 추가(conclusion 만, git 산출물
  없음), 머리말 목록·"대상 10개" → 11개. `tests/test_watch_schedule_health_window.py` ALL_TARGETS / CONCLUSION_ONLY_TARGETS 에 추가.
- `SCORECARD_PUBLIC_LEADERBOARD_WORK_ORDER.md` 표·Phase A 표기, `TASK_HISTORY.md` #201·#202 관련 서술에 → #203 표시.
  ⚠️ #202 커밋이 #202 항목을 **#172 본문 한가운데**(백틱 안 `## 진행 예정 (백로그)` 문자열 자리)에 끼워 넣어 #172 문장이 끊겨
  있던 것을 발견해 원위치(#201 뒤)로 옮기고 #172 문장을 복원함(내용 변경 없음).

**cron 근거 (§0-1 — 데이터로 재검토).**
- **원화 07:35 KST(`35 22 * * *`).** 코스피 입력은 전부 전날 저녁에 확정: `scrape.yml` 실제 종료 17:19~17:59 KST(09-02~04, GitHub
  자체 cron 이 늦게 발동한 날도 21:03~22:27), 결투 KR 기준값은 `workflow_run` 으로 그 직후(17:46~22:12). 자정 이후면 언제 돌아도
  "어제 값"이라 종전 07:30 은 데이터 관점에서 안전했고(#201 코스피 분석의 전제), 오너가 "굳이 옮길 이유 없으면 07:30 유지"라
  했으므로 근방 유지. 전날 밤으로 당기는 것은 발행일(`published_date` = 실행일)이 하루 앞으로 바뀌어 "다음 날 아침 발행" 전제가
  전부 흔들리므로 하지 않음. 07:30 이 아니라 07:35 인 이유는 정각·정30분 회피 관례(#202 테스트가 고정) 하나뿐. 미국 벤치마크
  (08:20)보다 앞이지만 이 트랙은 미국 파일을 읽지 않으므로 무관. 달러 트랙·결투 USD·워치독보다 앞.
- **달러 11:35 KST(`35 2 * * *`).** #202 의 값을 그대로 옮김. 2026-09-07 Actions API 재조회 — `scrape_report_snapshots.yml` 새 실행
  없음(최신 09-05 토 10:04 KST; 09-01~05 10:52·10:05·10:11·10:04·10:04), `scrape_us.yml` 09-02~05 실제 종료 08:20~08:39. 정상 범위
  실측 최댓값 10:52 + 30분 관례 = 11:22 가 하한이고 11:35 는 그보다 13분 뒤 — 이미 같은 계산으로 확정된 값이라 흔들지 않음.
  08-27~29(13~16시대)는 장애 기간 이상치라 기준으로 삼지 않음(그런 날은 검사가 "지수 원천 낡음 → 생략"으로 스스로 걸러 줌).
  테스트에 **상한**(벤치마크 cron 뒤 4시간 이내)을 추가해 "필요 이상으로 늦추지 말 것"을 고정.
- 충돌 확인: 22:35 / 02:35 UTC 에 다른 cron 없음(`render_keep_awake` 는 :03/:13/…). 워치독 18:00 > 11:35 > 07:35 순서 유지,
  `test_watchdog_cron_runs_outside_market_hours_and_after_every_target` 통과. 실측 GitHub 지연(1.5~5시간)으로 두 트랙이 겹쳐 돌아도
  통화별 행만 만지므로 안전(아래 §14 테스트가 그 근거).

**#5 — #202 가 남긴 월·화 `no_baseline` 빈틈: 닫음(한 조건 수준).** 결투 USD 배치가 토·일(·미국 휴장일) 대상으로 남기는 기준값
파일은 `index_keys: []`(그날 지수 종가가 없으니 당연). `load_freshness_inputs()` 가 "결투 기준선 날짜 ≠ 스냅샷 거래일"만 보고 그
파일을 골라 판정 함수가 `no_baseline` 을 냈음. 이제 **지수 없는 결투 기준선은 쓰지 않고 이력 CSV 직전 행으로 대체**(같은 날 규칙과
같은 대체 경로 — `_previous_probe_from_history()` 재사용, 결투 배치 로직·기준값 파일은 손대지 않음). 판정 함수의 "기준선에 지수
없음 → no_baseline" 동작 자체는 그대로(테스트 유지). 저장소 실데이터(2026-09-07 월 07:30 KST, US 세션 09-04, 결투 기준선 09-05
토 지수 없음)로 실행: **US → 검사함 → ok — 기준선 2026-09-03 (stock_history), 비교 48종목** (종전이면 `no_baseline` 생략).

**회귀 테스트 (`tests/test_scorecard_publish.py` §14 신설 + 기존 수정, 192 → 210).** 🔴 `test_single_currency_batch_never_touches_
the_other_currencys_rows[KRW-USD / USD-KRW]` — 모든 그룹에 과거 발행 행이 있는 가짜 클라이언트로 dry-run 아닌 실제 흐름을 돌려,
발행표 두 개에 나간 삭제·삽입이 건드린 통화가 담당 통화뿐임(`_touched_currencies()`: 삭제 필터·삽입 payload 로 판정, 통화 필터 없는
삭제는 `"*"` 로 잡힘) + 가지치기 select 조차 다른 통화 그룹엔 안 나감 + 당일 삭제 2건에 `in currency [담당]` 필터. `…with_nobody_
in_scope_prunes_only_its_own_groups[양방향]` — 담당 통화 보유자 0명(가장 위험한 모양)에서 담당 9개 그룹만 정리, `skipped` 비어 있음.
`both_currencies_default_still_sweeps_all_eighteen_groups`(하위호환), `all_possible_groups` 좁히기, 모르는/빈 통화 거절, DB 층 날짜
삭제 통화 필터·빈 목록 거절, 체급 배정도 담당 통화만, 신선도: 원화 배치는 미국 skip 을 판정조차 안 함 / 미국 배치는 코스피 skip·
**failed** 도 무시 / 담당 시장 skip 이면 그 통화만 스킵(원화 행 무사) / 실제 경로에서 `markets=(US,)` 로만 로더 호출 / H-3 거래일
확인이 담당 시장 기준 / `load_freshness_inputs(markets=)` 범위·모르는 시장 거절 / 지수 없는 결투 기준선 → 이력 행 대체(#5).
기존 수정: `test_one_skipping_market_skips_the_whole_day…` → `…skips_only_its_own_currency_and_publishes_the_other`(새 정책), 스킵
계열 4건은 실제 운영 모양대로 `currencies=[KRW]` 로. cron 테스트는 `test_usd_publish_cron_runs_after_us_benchmark…`(달러, 상한
추가) + 신규 `test_krw_publish_cron_is_an_early_morning_slot_that_does_not_wait_for_us_data`(원화: 벤치마크보다 앞·달러 트랙보다
앞·다른 분). 워크플로우 존재 테스트가 두 파일의 `ARGS="--currency X"` 고정(다른 통화 동반 금지).

**검증.** `pytest -q tests/test_scorecard_publish.py` **210 passed**, `tests/test_watch_schedule_health_window.py` **100 passed**,
결투·성적표 UI·스위트 무결성·문구 9파일 **738 passed / 73 skipped**, 나머지 33파일 **793 + 315 passed** — 전체 **2,156 passed / 73
skipped, 실패 0**(명령 시간 상한 때문에 5묶음). `run_scorecard_publish_batch.py --help` / `--currency EUR` 거절 확인, 워크플로우 3개
YAML 파싱 확인. 실제 GitHub 실행은 오프라인 검증 밖 — 첫 확인 포인트: 09-08(화) 07:35 원화 트랙 요약 "💱 담당 통화: KRW", 11:35
달러 트랙 요약 "💱 담당 통화: USD" + "🩺 … US: 검사함 → …(stock_history)"(월·화도 이제 판정됨). GitHub push 권한이 없는 세션이라
**로컬 커밋까지만** — 오너가 push.

**하지 않은 것 / 참고(§0-1).** (a) 결투 배치·기준값 파일은 한 글자도 안 씀. (b) `.github/workflows/` 는 원격 도구가 보호 경로로
막아(#5-15 전례) `device_bash` 로 tar 전달·검증(md5 일치)함. (c) 조사 중 확인 — 09-06(일) 16:10 에 `scrape.yml` 이 수동 실행돼
...
**후속 (같은 날, push 직후).** 오너가 작업 폴더를 `C:\Users\crown\.gemini\antigravity\scratch\visible_hand` 에서
`C:\Users\crown\visible_hand` 로 **폴더째 복사해 옮기고** 옛 쪽을 `visible_hand_OLD` 로 개명(제미나이 안티그래비티가 붙어 문제를
일으킨다는 판단). 그 과정에서 실제 사고가 한 번 났음 — 두 폴더가 서로 다른 커밋을 들고 있어, 세션이 새 폴더에 커밋한 #208 을 두고
오너는 옛 폴더에서 push 해 **#207(`f164cb8`)만 올라갔다.** 새 폴더에서 다시 push 해 `bf4e663` 반영 완료, GitHub Actions
"테스트 스위트" #40 **성공(2분 26초)** — 이 세션이 원격 VM 의 의존성 부재로 못 돌린 전체 스위트가 CI 에서 통과함을 확인.
새 폴더 점검: 안티그래비티·제미나이 흔적 0(숨김 항목은 `.claude`·`.git`·`.github`·`.devcontainer`·`.pytest_cache` 뿐), `git fsck`
정상(dangling commit 41개는 과거 rebase 잔재로 무해), 원격 설정 동일.
그러다 **`DIVIDEND_MODULE_WORK_ORDER.md` 에 옛 폴더 경로가 하드코딩돼 있는 것을 발견**(저장소 전체 전수 검색 결과 이 1곳뿐).
게다가 그 문서는 "git push 를 부탁할 때는 아래 두 줄을 **그대로** 주세요 — 다른 우회 방법을 새로 제안하지 마세요" 라고 못박아
둬서, 다음 배당 세션이 지시대로 따르면 오너에게 없는 폴더 경로를 주게 되는 구조였다(개명 덕에 지금은 조용한 오작동이 아니라
"지정된 경로를 찾을 수 없습니다" 로 막히지만, 그때마다 시간을 씀). 경로를 두 문서가 각각 들고 있으면 다음 이동 때 또 어긋나므로
**`CLAUDE.md` §0 을 단일 출처로 삼고**(폴더 경로 + cmd.exe 복붙용 push 두 줄 + 경로 변경 이력) 작업지시서는 그곳을 가리키게만 함(§0-3-10).
`Claude outputs/`(git 미추적, `git status` 에 `??` 로 계속 뜨던 폴더)는 내용 확인 후 오너 승인받아 삭제 — `duel_ledger_fix_2026-09-04_
backdated_monthly_deposit.sql` 은 `sql/` 의 추적 파일과 **md5 완전 일치하는 사본**이었고(원본 보존 확인), `usage_chart.png` 는 일회성
산출물. 처음엔 `.gitignore` 무시로 처리했다가(CLAUDE.md §4-7 "영구 삭제는 오너 확인") 오너가 삭제를 선택해 `.gitignore` 변경은 되돌림.
...
### #212 — [오너 지시 "A·B를 확실히 조정하고 C는 오푸스 엑스트라로 이어서 진행"] 네이버 신 증권 JSON API **실측 조사 완료 + 파서 모듈 신설** — 실제 응답 픽스처로 사보타주 9종 검증 (2026-09-07)

**경위.** 시작 전 HEAD = `4717900`. 오너가 네이버 개편 배너를 발견한 당일(#211) 저녁, **오너가 Chrome DevTools 로 신 사이트의 실제 요청·응답을 직접 복사해 주고 세션이 그것을 해석**하는 방식으로 조사했습니다. 세션은 웹 접근이 차단(`SITE_BLOCKED`)돼 있어 직접 받을 수 없었고, 우회 수단(curl 등)은 쓰지 않았습니다.

**조사 결과 — 대체 불가 항목이 하나도 남지 않았습니다.** #211 시점의 최대 병목이던 **추정 PER·EPS 가 신 API 에 그대로 있었고**, 이어서 새 병목으로 떠오른 **ROE·Forward ROE 는 "우리가 이미 매일 받고 있던 페이지"(`c1010001.aspx`) 안에 있었습니다.**

| 필요한 것 | 확보처 | 문서 |
|---|---|---|
| 종목 목록(시총순위) + **ROE**·EPS·PER·DPS·순이익 | `stock.naver.com/api/domestic/market/stock/default?orderType=marketSum` | §1-5-9·§1-5-10 |
| 추정PER·추정EPS·BPS·상장주식수 | `.../detail/<code>/detail?codeType=KRX` (인증 불필요 확인) | §1-5-4·§1-5-7 |
| **Forward ROE**·순이익·자본총계·EV/EBITDA·목표주가 | `navercomp.wisereport.co.kr/v2/company/c1010001.aspx` — **이미 수집 중** | §1-5-6 |
| 배치 시세 (한 요청 20종목) | `polling.finance.naver.com/.../stock/코드,코드,...` | §1-5-8 |

**🔴 오너가 잡아낸 함정 — NXT.** 오너 지적("NXT 넥스트트레이드 가격에 현재가가 계속 움직이고 있는 거 아닐까?")이 정확했습니다. 실측 결과 같은 저녁 SK하이닉스의 "현재가"가 **세 개**였습니다 — KRX 종가 1,783,000 / 애프터마켓 1,780,000 / 목록 API(`tradeType=NXT`) 1,784,000. 우리가 찾아낸 주소가 **전부 NXT 계열**이라 그대로 옮겼으면 종가가 크롤링 시각에 따라 달라지고, **틀린 값이 아니라 "다른 시장의 값"이라 산티체크에도 안 걸릴** 뻔했습니다(§0-1 겉보기 정상). 현행 스냅샷 `price`=1,783,000 은 KRX 종가와 일치해 **지금은 정상**입니다.

**세션이 스스로 반증한 것.** §1-5-4 에서 "ROE 는 `eps ÷ bps` 로 계산 가능성"이라고 적었으므로, 적어놓고 넘어가지 않고 스냅샷 393종목으로 검증했습니다 — **중앙값 오차 24.2%, 1% 이내는 11종목뿐.** 계산 대체를 폐기했습니다(§1-5-5). 맞는 11종목을 근거로 "대체로 맞는다"고 썼으면 그게 §0-1 이 말하는 겉보기 정상입니다.

**조사 중 드러난 현행 결함 2건(§1-5-12) → 오너 결정(§1-5-13).**
- ① `dps` 가 분기·반기 배당 종목에서 **1회분만** 저장(377종목 중 **79종목, 21%**). 배율이 정확히 4.00 인 종목이 다수(현대차·POSCO홀딩스·KT·CJ제일제당). 화면에 연간인 것처럼 표시되고 `total_dividend_krw` 가 75% 과소. **점수는 무사**(`sh_return` 은 배당수익률을 쓰고 그 값은 신 API 와 일치). → **오너 결정 A**: 신 API 값으로 덮어쓰지 않고 **자체 배당 모듈(DART·KIND) 실측을 1차 출처로** 재설계. `dividend` 에이전트 소관으로 **별도 분리**.
- ② 급등에 의한 배당 점수 잠식. 동일 199종목 한 달 비교 1.44%→1.35%, 19.6%가 10%+ 하락. **방향은 오너 지적대로**지만 히스토리가 25일치뿐이라 크기는 미확정. → **오너 결정 B**: **급등장 보정 안 함.** 배점은 SPEC §5 값이라 세션이 임의 변경 불가.
  ⚠️ **오독 정정 기록**: 전체 중앙값 1.31%→0.84% 급락을 급등 탓으로 볼 뻔했으나, 그날 수집 종목이 **201→500 으로 늘어난 유니버스 확대**가 원인이었습니다. 다른 집합끼리 비교한 것이라 무효입니다.

**만든 것 (이관 1단계, §6).**
- `utils/naver_stock_api.py`(380줄) — **`requests` 를 import 하지 않는 순수 파서.** HTTP·딜레이·재시도는 호출부 몫으로 분리해, 실제 응답 픽스처만으로 전량 테스트됩니다. 구 `fetch_naver_item_dps_and_eps()` 와 **키 집합을 똑같이** 맞춰 소비부를 안 고치고 출처만 갈아끼울 수 있게 했습니다.
- `tests/fixtures/naver_new_api/` — 🟢 **합성이 아니라 실제 응답**(#210 의 구 HTML 픽스처와 다른 점). 사람이 복사한 값이라 **전사 오류**가 위험이므로, 픽스처 자체의 산술 정합(`per == 가격÷eps` 등)을 검사하는 테스트를 먼저 두었습니다. `README.md` 에 출처와 **제외한 ETF 전용 null 필드 14개**를 밝혔습니다(§0-1).
- `tests/test_naver_stock_api.py` — 16검사.

**코드가 강제하는 함정 4가지**: ① NXT 주소 예외 차단(경로·쿼리·대소문자) ② `krxEps`(62,044)를 `eps`(224,313)로 오독 금지 — 3.6배 오차 ③ `dividendRate` 는 비율이 아니라 **퍼센트** — 1000배 오차 ④ 목록 `pbr` 은 **전일종가 기준**이라 결과 dict 에 `t_pbr` 키를 **아예 만들지 않음**(전일종가 ÷ 목록pbr == 상세 bps 로 수치 확인).

**검증.**
- 🔴 **사보타주 9종 전부, 의도한 테스트가 잡았습니다.**
- 회귀 대조: 목록 10종목×5항목 **43 일치 / 2 불일치**, 상세 7항목 **5 일치 / 2 불일치**. 불일치 원인 전부 규명 — (a) 배당수익률 0.168 vs 0.17 은 **현행의 소수점 2자리 반올림**, (b) `f_per` 5.1 vs 5.0 은 **원인 미확정**으로 남김. 현행 `f_per` 이 `현재가÷추정EPS` 와 맞는 종목이 258개 중 88개(34.1%)뿐이라 **현행 값은 네이버 표시값**임이 드러났습니다 → 이관 시 계산값으로 바꿀지는 `f_pegy` 에 들어가는 값이라 **오너 판단**.
- 전체 `pytest --ignore=archive`: **2,205 passed / 24 failed / 2 errors**. 24건은 원격 VM 에 `fastapi`·`FinanceDataReader` 가 없어서 나는 기존 실패이며, **`git stash` 로 이번 변경을 통째로 뺀 뒤 같은 파일을 돌려 결과가 완전히 동일함을 확인**했습니다. **CI 확인 필요.**

**작업 중 실수 1건(기록).** 사보타주 결과를 자동 추출하는 grep 패턴이 틀려 4건이 "안 잡힘"으로 보였습니다. 결론을 적기 전에 하나를 직접 열어보니 테스트는 정상 작동했고, 이 저장소 하네스가 `check()` 실패를 **teardown 오류로 승격**하는 것을 grep 이 놓친 것이었습니다. §0-1 — 확인 전에 "테스트가 안 잡는다"고 적지 않았습니다. (#210 에서도 같은 종류의 착각이 있었습니다 — 사보타주는 **결과를 믿기 전에 사보타주 자체가 유효한지** 먼저 봐야 합니다.)

**하지 않은 것(§0-1).** (a) **`collector_kospi200.py` 에 배선하지 않았습니다** — §0-3-6, 오너 승인 사항. (b) `c1010001.aspx` 파서 확장을 하지 않았습니다 — **실제 HTML 픽스처가 선행 조건**이고 세션은 받아올 수 없습니다(오너 저장 필요). (c) `cF1001.aspx` 는 **후보에서 제외**했습니다 — `encparam` 토큰이 필요해 토큰 추출이 곧 우회이고, 같은 데이터가 토큰 없는 페이지에 있습니다(§0-3-2). (d) 배치 API 의 **최대 크기를 시험하지 않았습니다** — 한도를 찾겠다고 큰 숫자를 넣어보는 것은 상대 서버에 대한 실례입니다(§0-3-2). 화면이 쓰는 20개를 따릅니다. (e) `tradeType=KRX` 존재 여부, `requests` 호출 시 헤더 필요 여부는 **미확인**으로 남겼습니다.

### #213 — [오너 지시 "만들어 둔 거는 만들어 둔 것대로 데이터가 잘 받아지는지 10일까지 데이터 축적도 해봐야 하지 않아?"] 네이버 신 API **섀도 관찰** 신설 — 실전 미접촉, 매일 대조 (2026-09-07)

**경위.** #212 로 파서를 만들었지만, **그 파서를 전환 당일(9/10)에 처음 돌려보는 것은 안전주의가 아닙니다.** 오너 지적대로 구 출처가 살아 있는 동안 신 API 를 나란히 받아 매일 대조해 두면, 전환하는 날 "이미 며칠째 같은 값이 나오고 있다"는 근거를 들고 갈 수 있습니다.

**만든 것.** `run_naver_api_shadow.py`(351줄) · `.github/workflows/naver_api_shadow.yml` · `tests/test_naver_api_shadow.py`(16검사) · `data/naver_api_shadow/`.

**🔴 실전 데이터 미접촉을 코드가 보증합니다.** `_assert_shadow_path()` 가 모든 쓰기 경로를 검사해 섀도 폴더를 벗어나면 예외로 멈춥니다. 실전 스냅샷(`data/kospi200_pegy_latest.json`)은 **읽기 전용**으로만 열며, 테스트가 소스 코드에서 쓰기 호출이 없는지까지 확인합니다. 화면·점수·스냅샷 영향 **0**.

**🟢 §0-3-2 매너 장치 (전부 테스트가 지킴).** 2.0~3.0초 랜덤 딜레이 · **순차 요청만**(`asyncio`·`ThreadPool` 미사용을 테스트가 확인) · **403/429 재시도·우회 없이 즉시 중단** · 연속 실패 5회 서킷 브레이커 · 1회 실행 요청 상한 60건 · `pageSize` 는 화면이 쓰는 **20 그대로**(한도 탐색 금지) · 상세는 전 종목(520)이 아니라 **표본 20종목**(관찰이 목적) · **안전망 cron 없음**(`scrape.yml` 완료 이벤트에만 연결 — 하루 걸러도 잃는 것이 없고, 필요 없는 요청을 보내지 않는 편이 낫습니다). 하루 약 **45요청**. `ENGINEERING_SPEC.md` §0-3-2 외부 의존성 목록에도 새 출처 2건(신 API·WiseReport)을 추가했습니다.

**알림.** 필드별 일치율이 95% 아래로 떨어지거나 공통 종목이 100개 미만이면 디스코드. 🔴 **판정 임계값은 파이썬 한 곳에만** 두고 YAML 은 전달만 합니다(§0-3-10) — 테스트가 YAML 에 임계값이 다시 적히면 빨간불을 냅니다.

**이 관찰이 답을 줄 미확인 2건.** ① `requests` 호출에 헤더가 필요한가(403 이 나오면 그 자체가 답이고, **우회하지 않고 중단 후 기록**) ② 목록 API 에 `tradeType=KRX` 가 실제로 있는가.

**검증.** 사보타주 9종 전부 잡힘(딜레이 축소 / 403 재시도 / 서킷 무력화 / 섀도 경로 검사 무력화 / 딜레이 호출 제거 / URL 을 NXT 로 / 실패 페이지 조용히 버림 / 요청 상한 제거 / YAML 에 임계값 재기입). ⚠️ **배치 사보타주 스크립트가 ②(403 재시도)를 "안 잡힘"으로 표시했으나, 직접 다시 돌려보니 정상 작동**했고 스크립트의 문자열 전달이 문제였습니다 — #210·#212 에 이어 **세 번째 같은 착각**입니다.

**하지 않은 것(§0-1).** (a) **세션이 실제로 네이버에 요청하지 않았습니다** — 세션의 웹 접근이 차단돼 있고 우회 수단을 쓰지 않습니다. 테스트는 전부 목(mock)이며, **실제 수집은 GitHub Actions 가 합니다.** 첫 실행 결과는 아직 없습니다. (b) ⏳ 이 워크플로우는 **한시적**입니다 — 이관이 끝나면 워크플로우·스크립트·데이터 폴더를 함께 정리하세요.

### #214 — [#213 후속, 오너가 실제 HTML 저장] WiseReport `c1010001.aspx` 파서 신설 — **Forward ROE 실측 출처 확보**, 사보타주가 검사 구멍 1개를 찾아냄 (2026-09-07)

**경위.** #212 에서 "실제 HTML 픽스처가 선행 조건"이라고 남겼는데, **오너가 브라우저에서 직접 저장해 주셨습니다**(`wisereport_c1010001_000660.html`, 267 KB, 실제 응답). "웹페이지, 완전"으로 저장돼 JS·CSS 1.6 MB 가 딸려왔고, HTML 한 장만 남기고 나머지는 `_to_delete/` 로 옮긴 뒤 `.gitignore` 에 넣었습니다.

**만든 것.** `utils/wisereport_parser.py` — **네트워크를 모르는 순수 파서**. `t_roe`·`f_roe`·당기순이익(지배)·자본총계(지배)·EPS·BPS·현금DPS·발행주식수·EV/EBITDA 를 돌려줍니다. `tests/test_wisereport_parser.py`(12검사).

**✅ 현행 수집값과 완전 일치.** `t_roe` 44.15 / `f_roe` 101.67 / `ev_ebitda` `"19.51"`(문자열 표기까지) — 셋 다 `data/kospi200_pegy_latest.json` 의 000660 값과 같습니다. **§1-5-5 에서 반증했던 "ROE 계산 복원"은 완전히 불필요해졌습니다.**

**코드가 지키는 것.** ① `iloc` 위치 인덱스 금지(§2-1) — 헤더를 `DataValidator.classify_header_timeframe()` 으로 분류(기존 `_fetch_ev_ebitda()` 가 2차 감사 1-6 이후 쓰는 방식 그대로). ② 🔴 **분기 컬럼 절대 미사용** — 같은 표에 연간 4열·분기 4열이 나란히 있고 **분기 ROE 는 "직전 4개 분기 연환산"**(실측: 연간 44.15 vs 분기 92.68). ③ 추정(E)과 실적 분리, 출처 컬럼명도 함께 반환. ④ 못 구하면 None + 사유.

**🔴 같은 이름 다른 기준(재확인).** 이 표의 `EPS(원)` 는 연간 확정 **58,955**, 신 JSON API 의 `eps` 는 최근 4분기 **224,313** — **3.8배** 차이. 테스트가 못 박습니다.

**사보타주 8종 — 그중 2건이 의미 있었습니다.** 6종은 즉시 잡혔고, 나머지 2건이 "안 잡힘"으로 나왔는데 성격이 달랐습니다: **하나는 제 사보타주가 무효**(`ANNUAL_ESTIMATE_KINDS` 에 `TTM` 추가 → `if/elif` 순서상 아무것도 안 바뀜, #210 H3 와 같은 종류)였고, **하나는 진짜 검사 구멍**("표는 있는데 재무요약 표만 없는" 경우 — 사이트가 구획을 옮기면 실제로 이렇게 됩니다)이었습니다. **검사 2건을 새로 추가**해 메웠습니다. 🔴 통과만 보고 끝냈으면 몰랐을 구멍입니다.

**하지 않은 것(§0-1).** (a) **배선하지 않았습니다** — `_fetch_ev_ebitda()` 그대로. 배선할 때는 그 함수를 이 모듈 호출로 **대체**해 같은 페이지를 두 곳에서 파싱하는 상태를 만들지 마세요(§0-3-10). 테스트가 수집기에 `wisereport_parser` 가 아직 없음을 확인합니다. (b) **픽스처가 000660 한 장뿐**입니다 — 적자·무배당 종목의 실제 페이지가 있으면 부호·빈칸 경로를 실제로 검증할 수 있습니다(지금은 합성 HTML 로 대신).

### #215 — [섀도 1회차 실행] 미확인 2건 해소 + **`startIdx` 를 잘못 안 것을 실측으로 발견·정정**, 같은 종류를 코드가 잡도록 검사 신설 (2026-09-08)

**경위.** #213 의 섀도를 오너가 수동 실행(`Naver API Shadow #1`, 07:20 KST). 결과가 저장소에 커밋돼 세션이 직접 읽었습니다.

**✅ 미확인 2건 해소.** ① **헤더 불필요** — `requests` 로, 봇임을 밝힌 정직한 User-Agent 로 요청 29건 전부 `200`. ② **`tradeType=KRX` 실재 확인.** 그리고 필드 일치율이 **현재가·ROE·Trailing EPS·Trailing PER·상장주식수 전부 100%(불일치 0건)** 였습니다 — 파서 자체는 실전과 완전히 같은 값을 냅니다.

**🔴 그러나 `startIdx` 의 의미를 제가 잘못 알았습니다.** 항목 오프셋이 아니라 **페이지 인덱스**(실제 오프셋 = `startIdx × pageSize`)였습니다. 0,20,40… 으로 요청한 결과 `startIdx=20` 이 **401위부터**, `startIdx=40` 이 **801위부터**를 돌려줬고, `startIdx=160`(=3201위)에서 빈 배열이 와 조기 종료했습니다. 그래서 "상위 500" 대신 **1~20위 + 401위 이하 140개**를 모았고, 실전과 겹치는 종목이 **40개**뿐이었습니다. 실측 확인(오너): `startIdx=1&pageSize=20` → 첫 종목 **하나금융지주**(시총 21위). **추론이 들어맞아도 확인 전에는 고치지 않았습니다**(§0-1).

**🔴 이 사고가 무서운 이유 — 겉보기엔 전부 정상이었습니다.** 첫 페이지가 맞았고, **전체가 시총 내림차순**이었고(정렬 검사로는 못 잡음), 모든 요청이 200, 오류 0건, 파싱 값 일치율 100%. 틀린 것은 "어떤 종목을 모았는가" 하나뿐이었고 **사람이 값을 눈으로 훑어 찾았습니다.** 다음에도 통할 방법이 아닙니다.

**처방 — `check_pagination_continuity()` 신설.** ① 페이지 **경계에서만** 나타나는 비정상 낙폭 ② 중복 종목(페이지 겹침) ③ 수집량 부족. 🔴 **절대 임계값을 쓰지 않았습니다** — 시총 상위권은 원래 낙폭이 큽니다(실측: SK하이닉스 1,302조 → 삼성전자우 160조 = **8.1배**, 진짜 시장 분포). 절대 임계값이면 여기서 오탐이 나고 **오탐이 나는 경보는 곧 무시당하는 경보**입니다. 대신 **같은 구간 내부의 정상 낙폭과 비교**합니다 — 상위 페이지는 내부 8.1배 → 임계 24.3배 → 실제 경계 **58배** 잡힘, 하위 페이지는 내부 ≈1.0배 → 임계 5배(하한) → 정상이면 조용. 실제로 깨졌던 응답을 **회귀 픽스처**(`tests/fixtures/naver_new_api/broken_pagination_2026-09-08.json`)로 보존했습니다.

**사보타주 8종 — 그중 3건이 또 검사 구멍을 찾았습니다.** 처음엔 3건이 "안 잡힘"으로 나왔고 **셋 다 진짜 구멍**이었습니다: ⓐ **URL 상수만 검사**해서 호출부가 `page × pageSize` 를 넘기는 실수를 못 잡았습니다 — **그게 이번 사고 그 자체**입니다 ⓑ `collect()` 가 경고를 **버려도** 검사가 없었습니다(§0-1 — 로그만 남기는 건 조치가 아님) ⓒ **오탐 방지가 검증되지 않았습니다**(상위권 큰 낙폭이 하필 경계에 놓이는 경우). 검사 3건을 추가해 메웠고 **8/8 전부 잡힙니다**. 테스트 16 → **24검사**.

**하지 않은 것(§0-1).** (a) 수정 후 **아직 재실행하지 않았습니다** — 오너 푸시 후 재실행해 500종목이 제대로 오는지 확인해야 합니다. (b) 요청량은 그대로입니다(25페이지 + 상세 20 = 45요청, 상한 60 안). (c) 목록에 리츠(RT)·뮤추얼펀드(MF)·DR 등이 섞여 옵니다(실측: ST 154 / RT 3 / MF 1 / FS 1 / DR 1) — 실전은 `kr_ticker_master` 로 ST 만 거르므로 **배선 시 같은 필터가 필요**합니다. 이번에는 손대지 않았습니다.

**#215 후속 — 섀도 2회차 (2026-09-08 07:37).** `startIdx` 정정 후 재실행. 수집 **160 → 500종목**, 공통 **40 → 499종목**, 요청 45건 **전부 200**, 페이지네이션 경고·오류 **0건**. 필드별 일치율 **현재가 499/0 · ROE 480/0 · Trailing EPS 401/0 · Trailing PER 401/0 · 상장주식수 499/0 — 전부 100%, 불일치 단 한 건도 없음.** ("비교불가"는 한쪽에 값이 없는 경우 — ROE 19건은 신규상장 등, EPS·PER 98건은 적자 종목으로 신 API 가 `per: null` 을 주며 현행 스냅샷도 같은 종목이 비어 있어 서로 어긋난 것이 아님.)
🟡 **배선 전 할 일 1건 발견**: 목록 API 가 `marketType=ALL` 이라 **비주식이 섞여** 옵니다(실측 ST 490 / RT 5 / IF 2 / DR 2 / MF 1). 그 10종목이 자리를 차지해 주식이 490개만 들어왔고 실전 500 중 21종목(전부 시총 4.7~4.8천억, 500위 경계 근처)이 빠졌습니다. 배선 시 `type == "ST"` 필터 + 여유 페이지가 필요합니다 — 현행도 `kr_ticker_master` 로 같은 필터를 하므로 새 개념은 아닙니다. **섀도에는 넣지 않았습니다**(필터를 넣으면 "신 API 가 무엇을 주는가"라는 관찰 대상 자체가 가려집니다).
→ **신 API 쪽 미확인은 남아 있지 않습니다.** 남은 것은 배선 판단(오너 승인)뿐입니다.

**#215 정정 — "비주식 필터가 필요하다"는 결론은 틀렸습니다 (오너 지적, 2026-09-08).** 목록 API 응답에 `type` 이 `ST` 가 아닌 종목이 섞여 오는 것을 보고(ST 490 / RT 5 / IF 2 / DR 2 / MF 1) "배선 시 `type=="ST"` 필터가 필요하다"고 적었는데, 오너가 *"리츠하고 인프라투자회사는 주식 종목으로 쳐도 되는 거 아냐?"* 라고 짚었습니다. **확인해 보니 현행이 이미 전부 수집하고 있었습니다** — `kr_ticker_master.json`(FinanceDataReader `StockListing('KRX')`)은 **STOCK/ETF 두 갈래만** 나누고(2,873/1,167), 맥쿼리인프라·SK리츠·롯데리츠·한화리츠·ESR켄달스퀘어리츠·신한알파리츠·맵스리얼티·KB발해인프라·코오롱티슈진·인제니아테라퓨틱스가 **전부 STOCK 으로 분류돼 스냅샷에 있고 화면에도 노출**(`is_visible=True`)됩니다. 지표가 없는 종목은 **거르는 게 아니라 검증에서 막습니다**(맥쿼리인프라: `is_valid=False`, 배지 "⚠️ 데이터 검증 필요", `quant_score=None`) — §0-1 그대로 "지어내지 않고 못 구했다고 보여주는" 설계입니다.
🔴 **필터를 넣었으면 §0-3-10 위반이었습니다** — 종목 유형 판정이 `kr_ticker_master` 와 네이버 `type` **두 곳**이 되고, 둘은 **실제로 어긋납니다**(FDR: STOCK ↔ 네이버: RT). 어긋나는 순간 리츠·인프라 10종목이 조용히 사라집니다. `type` 은 `api_security_type` 으로 참고 보관만 하며, **테스트가 필터 사용 자체를 막습니다**(`test_security_type_is_never_used_as_a_filter`).
🟢 **"21종목 빠짐"도 필터 탓이 아니었습니다.** 실측: 실전 시총 **상위 490 까지는 섀도 500 과 100% 일치**(빠짐 0), 상위 500 중 빠진 건 **정확히 500위 1종목**(경계 한 칸). 21종목 차이는 실전이 **520종목(상위 500 + 히스테리시스 버퍼 20)** 을 추적하는데 섀도가 500만 받아 생긴 순수한 경계 문제였습니다. → `LIST_TARGET_COUNT` 를 **520**(26페이지, 총 46요청, 상한 60 안)으로 고쳤습니다. 사보타주 2종(필터 삽입 / 목표 500 회귀) 모두 잡힙니다. 테스트 24 → **26검사**.
📌 **교훈**: "네이버가 비주식이라고 부르니 비주식이겠지"가 아니라 **우리 시스템이 이미 무엇을 하고 있는지 먼저 봤어야** 했습니다. 오너가 짚지 않았으면 리츠·인프라 10종목을 소리 없이 잃는 코드를 배선 지침으로 남길 뻔했습니다.

**#215 후속 2 — 섀도 3회차(2026-09-08 07:59, 개장 전): 불일치 0건 + 코넥스 혼입 발견.** `LIST_TARGET_COUNT` 520 으로 재실행. 요청 **46건 전부 200**, 수집 **520종목**, 페이지네이션 경고·오류 **0**, 개장 전 실행 확인. 대조 결과 **공통 516종목, 현재가·ROE·Trailing EPS·Trailing PER·상장주식수 전부 100% 일치(불일치 0)**. 실전 시총 **상위 516까지 빠짐 0**이고 나머지 4종목(517~520위)은 섀도 520번째와 같은 시총 구간(4.4~4.6천억)이라 순수한 경계 흔들림. 리츠·인프라도 그대로 포함됨(RT 6 / IF 2 / MF 1 / DR 2) — 정정된 방침대로.
🟡 **다만 섀도에만 있는 4종목 중 하나가 경계 문제가 아니었습니다** — `본시스템즈`(시총 1조 393억)가 **코넥스(KONEX)** 종목이었습니다. 섀도 520종목의 시장 구분: KOSPI 333 / KOSDAQ GLOBAL 45 / KOSDAQ 141 / **KONEX 1**. 현행은 `sise_market_sum.naver?sosok=0`(코스피)+`sosok=1`(코스닥)만 긁어 **코넥스를 수집한 적이 없습니다.** 신 API 의 `marketType=ALL` 은 코넥스까지 줍니다. 본시스템즈의 `t_roe` 는 **-426.33** — 코넥스는 유동성이 낮아 지표가 극단으로 튑니다.
🔴 **리츠 케이스와 정반대이지만 판정 기준은 같습니다.** 리츠·인프라는 **현행이 이미 수집 중**이라 거르면 회귀이고, 코넥스는 **현행이 수집한 적 없는 시장**이라 넣으면 회귀입니다. **둘 다 `kr_ticker_master.json` 이 답을 갖고 있습니다**(유형 STOCK/ETF + 시장 KOSPI/KOSDAQ/KONEX). 신 API 의 `type`·`sosok` 으로 **두 번째 판정을 만들지 않습니다**(§0-3-10) — `api_security_type`·`api_sosok` 으로 **참고 보관만** 합니다. 다만 **보관은 해야 합니다**: 이 필드가 없었으면 코넥스 혼입을 발견하지 못했습니다. 테스트로 못 박음(`test_market_and_type_fields_are_kept_for_reference_only`).
📌 배선 시 종목 선별은 **현행 그대로 마스터**로 합니다. 섀도에는 필터를 넣지 않습니다 — 필터를 끼우면 "신 API 가 실제로 무엇을 주는가"라는 관찰 대상이 가려지고, **코넥스를 발견한 것도 필터가 없었기 때문**입니다.

### #217 — [오너 지적 3건] 섀도를 **"넓게 표본"에서 "좁게 매일 전부"로** 전환 + 정제 검사 4종 신설 (2026-09-08)

**경위.** #216 에서 표본을 확대·회전하도록 바꿨는데, 오너가 연달아 세 가지를 짚었습니다.

**① *"매일 다른 종목 100개를 쌓으면, 그 사이사이에 데이터가 바뀌었을 때 바뀐 데이터를 정리하는 것까지 오류를 잡을 수 있겠어?"*** → **못 잡습니다.** 히스토리 실측: `t_eps` 는 거의 매일 바뀝니다 — 실적 시즌 하루 **74종목**(2026-08-24), 평소 6~21종목. 회전은 "오늘 일치"만 알려줄 뿐 **어느 날 어긋났는지·왜 어긋났는지**를 못 짚습니다.

**② *"데이터 정제까지도 중요한데 크롤링해서 제대로 제 위치를 잡을 수 있을지 없을지도 봐야 할 것 아냐. 현재 국내주식은 위아래가 다 롤러코스터라서."*** → 실측: 시총 순위가 하루 중앙값 2~3계단, **최대 70계단**, **매일 1~7종목이 500위권을 드나듭니다.** 다만 신 API 의 순서 자체는 오늘 기준 **완벽**했습니다 — 순서 뒤바뀜 0건, 시총 동률 0건, **직접 계산(현재가×상장주식수)으로 재정렬해도 0/520 어긋남**. 그래도 하루 측정이므로 **매일 확인하도록 검사를 붙였습니다.**

**③ *"데이터 오염을 잡는 게 어렵기 때문에 이것저것 계속 안전막을 막고 있는 건데, 지금 매일 100개씩 받는 걸로는 그걸 커버할 수가 없다. 차라리 크롤링 종목을 시가총액 순위 200개로 해서 전체적으로 매일 받으면서 확인을 하는 게 맞아."*** → **오너 판단대로 전환했습니다. 폭 대신 깊이입니다** — 오염은 시계열로만 보이는데 회전이 그 시계열을 끊습니다.

**설계 변경.** 상세 100 회전 → **시총 상위 200종목 매일 전부**, 위즈리포트 20 회전 → **같은 200종목 매일 전부**. 목록은 520 전부 유지(순위·집합 검증용, 26요청으로 쌈). 하루 요청 146 → **426건(약 18분)**, 현행 1,040요청의 **41%** 수준. ⚠️ **201위 아래는 섀도가 보지 않습니다** — 모르는 게 아니라 **알고 두는 공백**이며, 목록 항목은 520종목 전부 계속 대조됩니다.

**신설 검사 4종.** ① `check_rank_integrity` — 받은 순서가 시총 내림차순인지 + **직접 계산으로 재정렬해도 같은지** ② `check_change_sync` — **값이 바뀔 때 신 API 와 실전이 같이 바뀌는가.** 한쪽만 바뀌면 갱신을 놓친 것인데 **값이 같은지만 봐서는 안 보입니다**(바뀌기 전엔 둘 다 옛 값이라 '일치') ③ `check_universe_drift` — 같은 종목 집합인가(경계 몇 종목은 정상, 5% 초과면 범위 오류) ④ `check_timing` — **크롤링 시작·종료 시각·단계별 소요·응답 시간**.

**④ 시간 기록(오너 요구 *"받아지는 시간까지 확인을 해야 하는 것도 지금 필요하니까, 크롤링 시작 종료시간"*).** 기록 항목: 시작·종료 시각(KST) · 총 소요 · 단계별(목록/상세/위즈리포트) 소요 · 응답 시간 중앙값·최대·가장 느린 주소 · **딜레이에 쓴 시간**(매너 장치가 실제로 도는지 확인용). 쓰임새 셋: **㉮ 이관 후 실전 소요 추정**(현행 500종목 20~30분) **㉯ 수집이 장 시작까지 걸치는지** — 백필 없는 수집기가 장중에 돌면 그 순간 가격을 종가로 저장하는 사고가 이 저장소에서 실제로 났습니다 **㉰ 응답 지연은 상대 서버 부하 신호**(§0-3-2, 느려지면 요청을 늘리지 않음).

**검증.** 사보타주 **9종 전부 잡힘**(상세/위즈리포트를 회전으로 되돌림 · 시간 기록 제거 · 응답 지연 검사 무력화 · 총 소요 검사 무력화 · 순위 검사 무력화 · 변경 동조 무력화 · 집합 정합 무력화 · 요청 상한 상향). 테스트 30 → **34검사**. `ENGINEERING_SPEC.md` §0-3-2 외부 의존성 목록의 요청량도 갱신했습니다.

**하지 않은 것(§0-1).** (a) **실전 수집 범위는 손대지 않았습니다** — `collector_kospi200.py` 는 여전히 매일 520종목 전부를 구 네이버에서 받습니다. 오너 발언의 "크롤링 종목 200개"는 **섀도가 깊게 볼 범위**로 해석했습니다(문맥상 "매일 100개씩 받는 것"의 대안). 실전 범위를 줄이라는 뜻이었다면 별도 지시가 필요합니다. (b) 바뀐 설정으로 **아직 실행하지 않았습니다** — 오늘 16:05 이후 정기 실행이 첫 회입니다.

### #218 — [오너 결정] 섀도를 **520종목 전부**로 되돌림 + **얼어붙은 데이터 검사** 신설 (2026-09-08)

**경위.** 바로 앞 #217 에서 "좁게 200, 매일 전부"로 바꿨는데, 오너가 **폭도 되돌리라**고 했습니다. ① *"언제 어떻게 애널리스트들이 정보를 업데이트할지 모르니까 500개의 종목을 매일매일 받으면서 차라리 데이터를 누적하는 게 크롤링의 목적이야."* ② 위즈리포트 범위를 물었더니 — *"이게 지금 테스트용으로 지켜보기 위해서면 100종목만 보는 게 맞는데, 앞으로 **준비를 다 해보기 위해서는 520개 전부**가 맞다고 생각은 하거든."*

**실측이 오너 쪽이었습니다.** 상위 200만 보면 **놓치는 것이 1/3**입니다 — 추정치(`f_eps`·`f_roe`) 변경 411건 중 **201위 이하가 134건(33%)**. `f_eps` 는 하루 6~43종목, `f_roe` 26~55종목, `f_per` 75~108종목이 바뀝니다. "이관 후 실제로 돌릴 조건을 그대로 시험한다"는 목적에서 200으로는 시험이 안 됩니다.

**변경.** `SHADOW_UNIVERSE_SIZE` 200 → **520**(상세·위즈리포트 모두), `MAX_REQUESTS_PER_RUN` 460 → **1100**. 하루 요청 426 → **약 1,066건(약 45분)**. 목록 26요청은 변경 없음.

**🔴 §0-3-2 — 대가를 숨기지 않고 적습니다.** 현행 수집기가 이미 매일 보내는 약 1,040요청에 섀도 1,066요청이 더해져, 이관이 끝날 때까지 상대 서버가 받는 총량은 그날 **약 2배**가 됩니다. 대신 지킬 것 — **순차 + 2~3초 간격으로 45분에 걸쳐** 나가고(뭉쳐 보내지 않음), **403/429 는 즉시 중단**(변경 없음), 그리고 ⏳ **반드시 한시적**입니다(이관이 끝나면 워크플로우째 삭제 — 이 문장을 코드 주석·테스트·작업지시서 세 곳에 박아뒀습니다).

**상한을 지키는 방식 자체를 바꿨습니다.** 기존 테스트는 *"요청 상한 500건 이하"* 라는 **절대값**으로 막았는데, 오너 결정으로 숫자가 올라가면 테스트의 숫자만 고치게 되어 자물쇠 구실을 못 합니다. 세 가지 **상대 조건**으로 바꿨습니다 — ① **상한이 계획 요청 수보다 과하게 크지 않을 것**(여유는 재시도분 10%까지) → 코드가 조용히 더 보내는 것을 막음 ② **한 번 실행이 최소 30분 이상에 걸쳐 나갈 것** → 병렬화하거나 딜레이를 줄이면 즉시 빨간불 ③ **섀도 범위가 실전 범위를 넘지 않을 것** → 실전이 안 보는 종목을 섀도가 긁는 것은 관찰이 아니라 새 부하.

**🔴 신설 검사 ⑤ `check_frozen_data` — "어제와 오늘이 통째로 같은가".** 오너 확인 요청(*"이거 들어가 있을 건데 안 들어가 있나?"*)으로 저장소를 훑은 결과 — 결투·성적표(소비자)에는 **있고**(#201~#203), `utils/data_sanity.py` 와 섀도에는 **없었습니다.** **2026-09-04 실전 사고(#195)** 가 바로 이 모양이었습니다: 수집기가 "오늘 날짜 라벨 + 어제 내용물" 스냅샷을 남기고 정식 수집을 건너뛰었는데, 전 종목 주가가 하나도 안 바뀌었는데도 `data_sanity` 는 조용했습니다 — 결측도 아니고 종목 수도 같고 중앙값 이동이 0 이라 **오히려 "너무 정상"** 으로 보였기 때문입니다(오너: *"9월 4일에는 크롤링 문제가 있었어서 그 부분은 수정을 했었을 거야"* — 커밋 `4662dec`, 09-04 23:04 KST. 원인은 고쳤지만 **감시망은 그대로**). 섀도에 같은 눈이 없으면 신 API 가 얼어붙은 값을 주는 날 **"일치율 100%"라는 가장 안심되는 숫자**가 나오면서 양쪽 다 틀린 상태가 됩니다. 판정 — 변동 **0종목** 🔴 알림(#195 와 같은 모양이라고 명시) / 변동 5% 미만 🟡 기록만(휴장일이면 정상) / 어제 자료 없거나 공통 50종목 미만이면 **판단하지 않음**(없는 것을 있는 척하지 않음, §0-1).

**검증 — 사보타주 6종 중 🔴 1건이 진짜 구멍을 찾았습니다.** 요청 상한 3000으로 부풀림 / 딜레이 0.2초 / 섀도 범위를 실전보다 넓힘 / `check_frozen_data` 를 항상 조용하게 / 🔴 을 🟡 로 강등 → **5종 즉시 잡힘.** 그런데 6번째 — `compare_with_production` 에서 `check_frozen_data(...)` 의 **결과를 버려도** 테스트가 **전부 통과**했습니다. 검사가 **있는 것**과 **불려서 읽히는 것**은 다릅니다(이 저장소에서 `collect()` 가 경고를 버리던 것과 같은 계열). 어제 파일까지 만들어 두고 대조 리포트에 빨간불이 실제로 실리는지 보는 `test_compare_actually_runs_the_frozen_check` 를 새로 추가했고 같은 사보타주를 다시 하니 잡힙니다. 테스트 34 → **37검사**.

**🟡 오너 결정이 필요한 것 — `utils/data_sanity.py` 에도 같은 눈이 필요합니다.** 매일 09:30 에 도는 저장소 전체 워치독인데 검사가 `row_count_drop`·`row_drop_ratio`·`row_surge_ratio`·`unusable_ratio`·`median_shift_ratio` 다섯뿐이고 **신선도 검사가 없습니다.** 9/4 사고가 이 워치독을 그냥 통과한 이유입니다. `data-foundation` 소관이고 **섀도와는 별개 작업**이라(§0-3-6) 이 작업에서 같이 고치지 않았습니다.

**하지 않은 것(§0-1).** (a) **실전 수집기는 여전히 한 글자도 안 바뀌었습니다** — `collector_kospi200.py`·`scrape.yml` 은 구 `finance.naver.com` 그대로이고, 신 모듈 참조 0건입니다(테스트가 확인). 배선은 오너 승인 사항(§0-3-6). (b) 바뀐 520 설정으로 **아직 실행하지 않았습니다** — 다음 정기 실행이 첫 회입니다. (c) `data_sanity` 는 위에 적은 대로 손대지 않았습니다.

### #219 — [오너 지시] "어제와 통째로 같은 데이터"를 잡는 눈을 **저장소 전체에** + 경고를 **수습 가능한 시각에** (2026-09-08)

**경위 ①.** #218 에서 섀도에 얼어붙은 데이터 검사를 붙이며 "결투·성적표에는 있고 `data_sanity` 에는 없다"고 보고했더니, 오너가 짚었습니다 — ***"결투 성적표, 사실 이 가격이에요, 전부다 적용이 안되던걸 찾았지."*** 다시 확인해 보니 셋 다 붙어 있긴 한데 **성격이 달랐습니다**: 결투(`duel_rules.check_crawl_freshness`)만 **내용**을 보고, "사실 이 가격이에요"와 `data_sanity` 는 **시각(`last_updated_at`)만** 봅니다. 2026-09-04 사고(#195)는 *오늘 날짜 라벨 + 어제 내용물*이었으므로 시각만 보는 둘은 **그냥 통과**시켰습니다.

**경위 ②.** 이어서 오너가 알림 시각을 짚었습니다 — ***"매일 오전 9시 30분이라는 시간은 시간이 참 애매한 시간대인데, 크롤링이 실패 했어도 수습할 수 있는 시간도 아니고, 그날 장은 시작 했으니까 장 종료까지 기다려야하고."*** / ***"문제가 생긴 것을 수습할 시간을 벌기 위한 경고문들인데 그 경고문들을 쓸 수가 없네."*** **맞습니다.** 판정은 수집 직후(16:05·17:00·05:35)에 이미 나와 있었고 09:30 은 **읽어주기만** 하는 시각이었는데, 하필 09:00 개장 뒤라 알아도 그날은 손쓸 수 없었습니다(다시 수집하면 장중 가격이 그날 종가로 저장되는 사고 — 이 저장소가 실제로 겪은 것). 그리고 ***"이부분을 미국장 한국장을 분리해서 돌려야하는게 맞는거라고 생각해 이게 커버가 안되"*** — 두 시장 마감이 12시간 가까이 다른데 알림은 하루 한 번이라 **어느 쪽이든 한쪽은 반드시 늦습니다.**

**오너 결정 3건(AskUserQuestion).** ① 공용화 모양 — ***"결투 파일은 손도 안 대고, 새 모듈이 결투 규칙을 불러다 씀"***. 오너가 되물은 *"공용으로 썼다가 하나가 망가지면 다른 애들까지 연쇄로 박살나는 그런일은 나지 않을까?"* 에 대해, 안 나누면 **한 곳만 고쳐지고 나머지는 조용히 틀린다**(9/4가 그것)는 반대쪽 위험을 같이 제시했고, 결투 테스트가 깔려 있어 실제로는 *"연쇄로 박살"*이 아니라 **"연쇄로 들킴"** 이 된다고 답했습니다(아래 검증에서 실증). ② `data_sanity` 방식 — ***"요약에 '내용 지문' 한 줄 추가"***. ③ 화면 — ***"감시망 판정 결과를 화면에 띄우기"***.

**만든 것.** `utils/data_freshness.py` **신설** — `judge_frozen()`·`compare_unchanged()`·`fingerprint()`·`is_frozen()`. 순수 계산, 표준 라이브러리만. `utils/data_sanity.py` — 요약에 **내용 지문** 한 줄 추가(파일 크기 거의 안 늘림) + **검사 ⑦ `frozen_content`** 신설 + 화면용 `frozen_notice()`. `run_naver_api_shadow.py` — #218 에서 따로 만든 판정을 **공용 모듈 호출로 교체**(판정기가 진짜 하나가 됨). `web/pages/pegy_page.py` — 얼어붙은 날 배너(**판정하지 않고 읽기만**, `load_json_file_async` 로 이벤트 루프 안 막음). `.github/workflows/watch_data_sanity.yml` — 09:30 cron → **각 수집기 완료 이벤트(`workflow_run`)**, cron 은 안전망으로만. 지시서 `DATA_FRESHNESS_WORK_ORDER.md` **신설**, `ENGINEERING_SPEC.md` **§0-3-16 신설** + **§0-3-15 정정**(#204 감사에서 사후 감시를 "대상 아님"으로 분류한 것이 틀렸음 — *감시도 소비자*이고, 판정 기준은 **"이 알림을 받은 사람이 지금 무엇을 할 수 있는가"**).

**🔴 정직하게 남기는 한계.** (a) **결투와 계산이 두 곳에 남았습니다.** 결투 규칙은 **지수 2개(KOSPI·KOSDAQ)를 앵커로 요구**하는데(휴장일과 수집실패를 구분하려고), 섀도·산티체크에는 지수가 없습니다. 지수를 새로 받으면 상대 서버 요청이 늘고(§0-3-2) 지어내면 §0-1 위반이라, 공용 모듈은 **앵커 없이 답할 수 있는 것까지만** 답합니다. 완전 통합은 결투 파일을 고쳐야 하므로 **오너 별도 판단**. (b) **휴장일에는 정상적으로 걸립니다**(연 10~15일). 사유 문장에 "휴장일이면 정상입니다"를 같이 적어 한 번 보고 넘기게 했습니다 — 놓쳐서 하루를 통째로 잃는 것보다 낫다는 오너 판단. (c) 지문은 "통째로 같다/아니다"만 알고 **몇 개가 바뀌었는지는 못 셉니다.** (d) 문제가 이어지는 동안 알림이 하루 3~4통 옵니다(같은 문제를 여러 번 알리지만 "아직 안 고쳐졌다"는 매번 참).

**검증.** 전체 **2,512검사 통과 · 실패 0**(로컬 VM 에 `fastapi`·`nicegui`·`google-genai`·FDR 를 설치해 이번 세션 처음으로 전 파일이 돌았습니다). 신설 `tests/test_data_freshness.py` **14검사**, `test_data_sanity.py` 39 → **51검사**, `test_pegy_page.py` 22 → **25검사**. **사보타주 26종 전부 잡힘**(공용 7 · 산티체크 6 · 섀도 4 · 워크플로우 5 · 화면 4). 그중 **🔴 1건이 진짜 구멍**이었습니다 — *"지문이 결측을 포함하는가"* 테스트가 **아무것도 안 지키고 있었습니다.** 숫자 하나를 `None` 으로 바꿔 비교했는데 그러면 어차피 숫자 묶음도 달라져서, 지문을 "숫자만" 내도 그대로 통과했습니다. **숫자 묶음이 완전히 같고 결측만 다른** 경우로 고쳐 쓰니 잡힙니다. 그리고 오너가 걱정한 연쇄를 **일부러 실증**했습니다 — 공용 모듈에서 `frozen` 판정을 없애는 사보타주를 넣자 **섀도 테스트가 같이 빨간불**이 됐습니다.

**하지 않은 것(§0-1).** 결투 파일 **무접촉**(위 (a)). 미국주식 화면·결투 화면에는 배너를 안 붙였습니다(이번엔 "사실 이 가격이에요"만). 휴장일 달력은 붙이지 않았습니다 — 붙이면 (b) 오탐이 사라지지만 외부 의존성이 하나 더 생기므로(§0-3-2) 별도 판단이 필요합니다. `collector_kospi200.py` 는 데이터셋 키를 `data_sanity.DATASET_KOSPI200` 상수로 바꾼 **한 줄**만 손댔습니다(화면과 같은 문자열을 보게 하려고 — §0-3-10).

### #220 — [오너 승인] 네이버 출처 전환 스위치 배선 — 구 경로 유지, 기본값 구 출처 (2026-09-08)

**경위.** 네이버 구 증권 종료(2026-09-10)를 이틀 앞두고 오너가 배선 방식을 정했습니다 — ***"구 경로를 지우지 않고 출처 전환 스위치로 붙이는 것(구 주소가 살아 있는 동안 양쪽 대조 가능, 문제 생기면 되돌리기 쉬움). 이 방식으로 가자."*** 준비는 이미 끝나 있었습니다(#212 JSON 파서 · #214 위즈리포트 파서 · #213~#219 섀도 관찰, 2026-09-08 07:59 실측 **516종목 5개 항목 전부 100% 일치, 불일치 0**).

**만든 것.** `utils/naver_source.py` **신설** — 환경변수 `NAVER_SOURCE`(`legacy`/`new_api`)를 읽는 **단일 출처**. 기본값·변수명이 저장소에서 이 파일에만 있고, 테스트가 다른 곳에 다시 적히지 않았는지 스캔합니다(§0-3-10). 오타(`new-api` 등)는 조용히 기본값으로 떨어지지 않고 **수집기가 시작하자마자 멈춥니다.** `collector_kospi200.py` 에 신 경로(`_fetch_market_list_new_api`·`fetch_naver_item_new_api`) 추가 — **구 경로 함수는 한 글자도 지우지 않았습니다.** 어느 출처로 만든 스냅샷인지 `metadata.data_source` 에 주소·거래소·종가 채택 조건까지 사람이 읽게 남깁니다(§0-1). `_fetch_ev_ebitda()` 는 `utils/wisereport_parser.py` 호출로 **대체**(§0-3-10) — 이건 스위치를 따르지 않습니다(종료 대상 도메인이 아니고 두 경로가 똑같이 씁니다).

**오너가 켜는 법.** `.github/workflows/scrape.yml` 의 `# NAVER_SOURCE: new_api` 줄에서 맨 앞 `# ` 두 글자를 지우고 커밋. 되돌리기는 다시 붙이기. **기본값은 구 출처**로 두었습니다 — 실전 전환은 오너 승인 사항(§0-3-6)이라 임의로 뒤집지 않았습니다.

**검증.** 전체 **2,541검사 통과**(기준선 2,512 → 신설 `test_naver_source_switch.py` 25검사 포함). **사보타주 20종 전부 잡힘**(기본값 뒤집기·출처 기록 제거·NXT 차단 무력화·오타 폴백·수집기 하드코딩·YAML 재기입 등). 구 경로 결과 불변을 파서·계산 기준선 2개 재생성 없이 확인했습니다.

**🔴 하지 않은 것(§0-1).** ①**실제 네트워크로 신 경로를 돌리지 않았습니다**(픽스처로만). ②`closePrice`+`localTradedAt 15:30` 검증은 **못 붙였습니다** — 우리가 쓰는 KRX 목록·상세 응답에 그 필드가 **없습니다**(실측). 대신 같은 응답의 `marketStatus` 가 전 종목 `CLOSE` 가 아니면 **수집 중단**으로 같은 뜻을 지킵니다. 다만 장마감 후 KRX `marketStatus` 가 `CLOSE` 인지는 **직접 보지 못했습니다**(개장 전 520종목 전부 `CLOSE` 실측만 있음). ③🔴 **스위치가 덮지 않는 구 주소가 하나 더 있습니다** — `run_kr_all_market_prices_collector()`(전 종목 종가 → 결투·성적표 현재가·보조지표 유니버스)가 여전히 `sise_market_sum.naver` 를 씁니다. 신 API 로 바꾸면 요청이 약 60→145로 늘어 **오너 판단이 필요해 별도 작업으로 남겼습니다.** 커밋 `6338ee1`.

### #221 — 상장종목 마스터 문지기 — 반쪽짜리로 정상 파일을 덮어쓰지 않음 (2026-09-08)

**경위 — 실제 사고에서 나왔습니다.** 2026-09-08 16:11 오너가 `Daily Market Scraper` 를 수동 실행했는데 실패했습니다. 로그 원문: `⚠️ StockListing('KRX') 실패: HTTP Error 404` → `StockListing('ETF/KR') 1167건 반영` → `1167건 저장 완료 -> data/kr_ticker_master.json` → `0 real KOSPI candidates` → `RuntimeError: 시가총액 목록 스크래핑 실패 — 수집을 중단합니다`.

🔴 **주식 목록이 404 로 실패했는데도 ETF 만 담아 마스터 파일을 덮어썼습니다.** 정상은 주식 2,873 + ETF 1,167 = **4,040건**인데 **1,167건(전부 ETF)** 짜리로 갈아치운 것입니다. 오늘은 그 뒤 단계가 실패해 커밋 스텝이 안 돌아 **저장소 파일이 우연히 살아남았습니다.** 다음에 FDR 이 또 404 인데 나머지가 성공하면 반쪽짜리가 커밋됩니다 — 그리고 #220 의 **신 경로는 종목 선별을 이 마스터 한 곳에 의존**하므로(`type=STOCK` 그리고 `market ∈ 코스피·코스닥`), 그 순간 후보가 **0건**이 됩니다.

**조치.** 원천 단위 판정 — `StockListing('KRX')`(주식)와 `StockListing('ETF/KR')`(ETF)가 **둘 다 예외 없이 1건 이상**일 때만 파일을 씁니다. 하나라도 실패·0건이면 **안 씁니다.** 근거(주석에 기록): ①파일의 `metadata.source` 가 "KRX + ETF/KR" 라고 적히므로 한쪽 빠진 내용을 그 이름표로 저장하면 파일 자체가 거짓말(§0-1) ②읽는 쪽이 전부 "마스터에 없으면 걸러냄" 규약이라 어느 쪽이 빠져도 어제 파일보다 나쁨 ③이 자리는 원인을 **직접 아는** 곳이라 건수 비교로 추정할 이유가 없음. `utils/data_sanity.py` 급감 판정을 안 쓴 이유도 적었습니다 — 그건 저장 **뒤**에 얹히는 관찰자이고 여기는 저장 **전** 문지기라 성격이 다릅니다(§0-3-10 — 같은 판정을 새로 짜지도 않음).

**수집 전체는 계속 갑니다.** 마스터는 보조 목록이라 어제 것이어도 그날 가격·밸류에이션 수집은 됩니다. 여기서 죽이면 보조 목록 하나 때문에 핵심 산출물을 통째로 잃습니다. 대신 **"로그만 남기는 것은 조치가 아니다"(§0-1)** 를 지켜 사실이 여러 경로로 드러납니다: 🚨 로그 → Actions 실행 요약 `::warning` → 코스피 스냅샷 `metadata.ticker_master` → `/kr` 관리자 배너(문장 생성은 `data_sanity.ticker_master_notice()` 한 곳).

**검증.** 전체 **2,555검사 통과**(2,541 → +14). **사보타주 9종 전부 잡힘.** 그 과정에서 기존 테스트 1건의 결함도 찾았습니다 — `test_kr_ticker_master_collector` 가 파일 전체에서 두 함수 이름의 **첫 등장 위치**를 비교하는 우연한 판정이라 주석에 이름이 한 번 더 나오자 뒤집혔고, 라벨도 사실과 달랐습니다. `__main__` 블록만 보도록 정정했습니다.

**🔴 FDR 404 에 대해 알아낸 것(코드만 읽고).** `StockListing('KRX')` 는 KRX 직접이 아니라 ①`data.krx.co.kr` 에 최신일자를 묻고 ②**FinanceData 의 GitHub 캐시 CSV**(`raw.githubusercontent.com/FinanceData/fdr_krx_data_cache/.../{날짜}.csv`)를 받습니다. 404 형태로 보아 **②캐시 CSV 부재**가 코드상 일관된 설명이고, 가장 그럴듯한 그림은 타이밍(16:11 KST 에 그날 캐시가 아직 없음)입니다 — **🔴 추정입니다.** 캐시 저장소 실제 상태·러너의 FDR 버전은 네트워크가 막혀 **확인 못 했습니다.** 그래서 재시도를 넣지 않았습니다(§0-3-2). ⚠️ **부수 발견**: `StockListing('ETF/KR')` 는 **`finance.naver.com/api/sise/etfItemList.nhn`** — **9/10 종료 대상 구 도메인**입니다. 그때부터 ETF 쪽도 실패할 수 있고, 그 경우 마스터는 이 문지기 덕에 마지막 정상 파일에서 멈추며 매일 경고가 뜹니다. **대체 출처는 미확인 — 별도 과제입니다.** 커밋 `b94b612`.

### #222 — 🔀 **신 출처로 전환** (2026-09-08, 오너 승인)

**결정 근거.** 같은 날 16:11 실행에서 구 순위 페이지가 **코스피 0건·코스닥 0건**을 돌려줬습니다. 오너가 브라우저로 확인한 결과 페이지 자체는 **정상으로 보였고**, 상단이 **"Npay 증권"** 으로 개편돼 있었습니다 → 페이지가 살아 있는데 **우리 파서가 읽던 표가 사라진 것**으로 보입니다. 그리고 같은 날 아침 섀도는 **같은 깃허브 서버에서** 신 API 45요청 전부 200 을 받았으므로 **IP 차단이 아닙니다.** (🔴 구 페이지 HTML 을 직접 열어보지는 못했습니다 — 세션의 웹 접근이 그 도메인을 막습니다. 정황이지 단정이 아닙니다.)

**조치.** `scrape.yml` 의 스위치를 켰습니다(`NAVER_SOURCE: new_api`). 켠 이유·되돌리는 법을 그 자리 주석에 날짜와 함께 남겼습니다.

**테스트 1건 정정.** `test_workflow_has_a_commented_switch_line_and_no_invalid_active_value` 가 *"주석 처리된 안내 줄이 **반드시** 있어야 한다"* 고 못 박고 있었는데, 그건 **스위치가 영영 꺼져 있어야 한다**는 뜻이 됩니다 — 켜자마자 걸렸습니다. 스위치는 켜지라고 만든 것이므로, 지켜야 할 것은 "꺼져 있음"이 아니라 **"켜져 있든 꺼져 있든 사람이 그 줄을 찾을 수 있고 값이 유효하다"** 입니다. 그렇게 고치고 **"켜진 줄이 두 개면 뒤엣것이 조용히 이긴다"** 는 검사를 새로 넣었습니다. 사보타주 3종(오타·두 줄·줄 삭제) 전부 잡힘.

**검증.** 전체 **2,555검사 통과, 실패 0.**

**🔴 이 전환으로 달라지는 것(결함 아님).** `dps` 가 연간값(구 경로의 분기 1회분 결함 대비 값이 커짐 — 오너 결정 A 의 자체 배당 모듈은 그대로 별도) · `f_per` 소수점 · `t_pbr` 숫자형 · 상세 403/429·장중 실행은 수집 통째로 중단. **첫 실전 실행 로그를 반드시 확인해야 합니다** — 신 경로는 아직 실제 네트워크로 돌아본 적이 없습니다.

### #223 — 🛑 섀도 자동 실행 정지 (2026-09-08, #222 직후)

**왜.** #222 로 실전이 신 출처로 넘어간 순간, 섀도의 임무가 끝났습니다. 섀도는 *"신 API 가 구 출처와 같은 값을 주는가"* 를 확인하는 물건인데, 이제 돌면 **신 API 로 받은 값을 신 API 로 만든 스냅샷과 비교**합니다 — 같은 파서(`utils/naver_stock_api.py`)를 **자기 자신과 대조**하는 것이라 얻는 정보가 **0** 입니다.

그런데 비용은 그대로입니다: **하루 약 1,066요청 · 45분.** 실전이 방금 같은 양을 보낸 직후에 이걸 또 보내면 그날 네이버가 받는 요청이 **두 배**가 됩니다. 얻는 것 없이 상대 서버에 두 배를 보내는 것은 §0-3-2 정면 위반이고, 오너 절대 원칙(*"상대방 서버에서 차단 당할 일을 절대로 만들 면 안된다"*) 위반입니다. **하필 지금은 차단당하면 대안이 없는 시점**입니다 — 구 출처가 이미 안 읽히니까요.

**조치.** `naver_api_shadow.yml` 의 `workflow_run` 트리거를 주석 처리했습니다(수동 실행은 남김 — 이관 후 문제 진단용). 왜 껐는지·되살리는 법·언제 통째로 지울지를 그 자리에 적었습니다.

**🔻 검사 구멍 발견.** 이 워크플로우의 **트리거를 지키는 검사가 아예 없었습니다** — 자동 실행을 꺼도 테스트가 하나도 안 울렸습니다(§0-3-15 배선이 보호되지 않고 있었음). `test_shadow_does_not_run_itself_automatically_after_the_cutover` 를 새로 넣어, 되살리면 여기가 걸리게 했습니다. 사보타주(트리거 복구)로 잡히는 것 확인. 테스트 37 → **38검사**.

**⏳ 남은 정리.** 이관이 완전히 끝나면(= `run_kr_all_market_prices_collector()` 까지 신 출처로 옮기면) 이 워크플로우와 `run_naver_api_shadow.py`·`data/naver_api_shadow/` 를 **통째로 지웁니다.** 필요 없어진 주기 작업을 남겨두는 것도 §0-3-2 위반입니다.

### #224 — 🔴 신 경로 상장주식수는 **목록 API 가 준 값**을 1차로 (2026-09-08, 전환 첫 실전 실행 사고 조치)

**경위 — #222 전환 후 첫 실전 실행(17:40)의 실제 로그.**
...
⚠️ 상장주식수를 찾지 못해 통합 순위 계산에서 제외된 종목 659개: 005930, 000660, ... 외 639개
RuntimeError: 상장주식수 매칭 실패로 통합 순위를 계산할 종목이 0개입니다 — 수집을 중단합니다
```

🟢 **신 API 는 완벽히 성공했습니다** — 659종목 수집, `marketStatus=CLOSE` 확인(#220 에서 "직접 보지 못했다"고 남겼던 미확인이 실전에서 닫혔습니다).
🟢 **#221 문지기가 첫 실전에서 실제로 일했습니다** — 로그: *"원천 일부가 실패해 파일을 쓰지 않습니다… 기존 파일(2026-09-07자)을 그대로 둡니다."* 안 만들었으면 그날 ETF 만 남은 마스터가 커밋됐습니다.
🔴 **막힌 곳은 FinanceDataReader 하나** — `_load_outstanding_shares_lookup()` 이 404 라 상장주식수가 빈 dict 가 됐고, `_rank_candidates_by_market_cap()` 이 659종목을 **전부** 순위 계산에서 제외해 0개가 됐습니다.

**핵심 — 받아놓고도 안 쓰고 있었습니다.** 신 API 목록 응답은 `listedStockCnt` 로 **상장주식수를 이미 줍니다**(`utils/naver_stock_api.parse_market_list_row`). 그런데 수집기는 그걸 두고 FDR 에 다시 물어보다 막혔습니다. 그 값은 검증돼 있습니다 — 섀도가 FDR 기반 실전 스냅샷과 대조해 **516종목 100% 일치**를 확인했습니다.

**조치 — 우선순위 ①후보 dict 가 가진 값(신 API) → ②FDR(①이 없는 후보가 하나라도 있을 때만 조회) → 둘 다 없으면 그 종목만 순위에서 제외.** 근거(주석에 기록): §2-3-1 *"재료는 받는 것"* — 출처가 **현재가와 같은 응답**에 실어 준 값을 두고 다른 서버에 다시 물어볼 이유가 없습니다. §0-3-2 — 평소 FDR 요청이 매일 1회 → **0회**로 줄었습니다. 폴백은 지우지 않았습니다(구 경로·값 없는 날용).

**구 경로 불변.** 분기 판단을 "어느 출처인가"가 아니라 **"후보가 그 값을 갖고 있는가"** 로 했습니다 — 구 경로 후보에는 그 값이 없으니 저절로 예전처럼 항상 FDR 입니다. 기존 순위 테스트 4건·특성화 기준선 무수정 통과. (🔻 정직하게: 구 경로 후보 dict 에 키 두 개가 **추가**됩니다. enrich 가 명시 키만 써서 스냅샷·계산 결과는 불변이지만 "한 글자도"의 문자 그대로는 아닙니다.)

**§0-1 — 어느 출처를 썼는지 남깁니다.** 스냅샷 `metadata.outstanding_shares_source` 에 `{priority, 출처별 종목 수, 제외 종목 수, fdr_attempted, fdr_lookup_count}` + 로그 한 줄.

**🔎 `skipped {'parse': 49}` 의 정체 — 빠진 종목은 0 이었습니다.** 코드 결함이 아니라 **카운터 라벨 결함**이었습니다. 예전 코드는 파서 `errors` 메모 수를 `parse` 로 셌는데, 그 메모에는 종목이 **유지된 채** 남는 배당 모순 기록(*"주당배당금 0.0 인데 배당수익률 X% — 확정하지 않고 미수집으로 둡니다"*, 의도된 §0-1 동작)이 섞여 있었습니다. 산술 근거: 후보 659 + 범위 밖 1 = **660 = 33페이지 × 20행**, 즉 받은 행이 전부 파싱됐습니다. `parse` 를 (받은 행 − 파싱된 행)으로 정정하고 메모는 `파서 메모 N건` 으로 따로 찍게 했습니다. 🔴 **그날 실제 응답 660행을 직접 보지는 못했습니다**(네트워크 차단) — 코드 경로와 산술로만 판정했고, **다음 실전 로그의 `'parse': 0; 파서 메모 49건` 으로 확인해야 합니다.**

**검증.** 전체 **2,570검사 통과**(2,556 → +14). 🔴 **오늘 사고를 그대로 재현하는 테스트**를 넣었습니다 — `test_incident_2026_09_08_new_api_survives_when_fdr_is_404`: FDR 이 로그 문구 그대로 404 를 내도 수집이 끝까지 성공하고, `market_cap = 현재가 × listedStockCnt` 이며, FDR 호출이 **0회**임을 확인합니다. **사보타주 8종 전부 잡힘**(순위가 후보 값 무시 / FDR 항상 조회 / 우선순위 뒤집기 / metadata 제거 / enrich 가 상세값 버림 등).

**🔴 하지 않은 것(§0-1).** FDR 404 **원인은 여전히 미해결**입니다(재시도·대체 출처 없음). 종목 단위 출처 표기는 안 했습니다 — enrich 출력 키를 늘리면 특성화 기준선이 깨져 오너 승인 사항입니다. 화면 배너도 안 띄웠습니다(메타데이터에는 있습니다). 커밋 `a0bbc12`.

### #225 — 🔭 외부 데이터 출처 감시 에이전트 `data-sources` 신설 + 첫 재고조사 (2026-09-08, 오너 지시)

**경위.** 하루에 외부 출처 **두 개가 동시에** 무너지고 나서 오너가 물었습니다 — ***"우리가 지금 데이터를 가져오는 곳을 API던 공식이던 뭐던 탐색하는 에이전트를 하나 만들고 작업을 시킬 수는 없을까?"*** 범위에 대해서는 ***"오픈소스던 뭐던 그런것들을 관리가 되고있는 곳이 있는지도 계속 조사를 해보고 관리를 하는 곳을 계속 탐색도 해봐야하니까."*** 그리고 이어서 ***"물론 법적인것이랑 비용이 안드는 방향도 무시하면 안되는거야."***

**왜 필요했는지가 그날 증명됐습니다.** 우리를 멈춰 세운 FinanceDataReader 가 **`ENGINEERING_SPEC.md` §0-3-2 의 "우리가 접속하는 대상" 목록에 아예 없었습니다.** 그래서 "이게 죽으면 뭐가 멈추지?"를 아무도 미리 생각해 본 적이 없었습니다. §0-3-12 가 *"외부 의존성은 방치하지 않고 능동적으로 점검한다"* 고 이미 못 박고 있었지만, 그 규칙이 **`requirements.txt` 패키지만 보고 데이터 출처는 안 보고 있었습니다.** 이번 작업이 그 구멍을 메웁니다.

**만든 것.** `.claude/agents/data-sources.md` **신설** — "우리가 밖에서 가져오는 모든 것"의 재고·건강·대체재 담당. `DATA_SOURCES_INVENTORY.md` **신설** — 첫 재고조사. `tests/test_data_sources_inventory.py` **신설**(6검사) — 목록이 다시 낡는 것을 테스트가 막습니다(오너 결정).

**🔴 재고조사 결과 — §0-3-2 목록에 빠져 있던 호스트가 12개였습니다.** `finance.naver.com` 의 지수·환율·수급 페이지, `fchart.stock.naver.com`(FDR `DataReader` 뒤 — **하루 1,000회 이상**), `data.krx.co.kr` + FinanceData 깃허브 캐시, `query2.finance.yahoo.com`(매크로의 `^KS11` 이 KRX 가 아니라 야후로 갑니다 — 소스 분기까지 읽어 확인), `opendart.fss.or.kr`, DART·Gemini·GitHub API 등. **FinanceDataReader 0.9.202 의 설치된 소스를 직접 열어** 실제 목적지를 확인했습니다 — URL 을 grep 한 결과만 믿었으면 이번에도 놓쳤을 것들입니다.

**🔴 폴백 없는 단일 장애점 10곳.** 그중 급한 둘: ①**`stock.naver.com` 신 API** — 코스피 화면 전체가 여기 얹혀 있는데 되돌릴 구 출처가 이미 죽었습니다. ②**`sise_market_sum.naver` 전 종목 종가** — `run_kr_all_market_prices_collector()` 가 **아직 신 API 로 안 옮겨졌고**, 09-08 16:11 에 이미 0건이라 `kr_all_market_prices.json` 이 09-07 자에서 멈췄습니다(보조지표 유니버스·성적표·결투·배당 화면의 유니버스 밖 시세가 여기 걸려 있습니다).

**🔴 개인·소규모 오픈소스 의존 3곳.** FinanceData 깃허브 캐시(마스터 파일) · `Ate329/top-us-stock-tickers`(미국 유니버스 첫 단계) · `yfinance`(⚠️ `requirements.txt` 에 없어 Actions 에서 **PER 교차검증이 죽은 코드**입니다 — "검증 통과"가 아니라 "검증 안 됨").

**⚖️💰 오너 지시로 추가한 1순위 기준.** 대체재는 *"되느냐"* 가 아니라 ***"써도 되느냐 · 공짜냐"*** 부터 봅니다 — ⚖️법적(약관·robots·재배포/상업적 이용·라이선스) · 💰비용(무료 한도·유료 전환·카드 요구) · 📜공식성(공식 API 인가 화면 뒤 내부 주소인가). **하나라도 확인 못 하면 "후보"가 아니라 "확인 필요"** 이고, **비용이 드는 것은 오너 결정 없이 후보로 올리지 않습니다.** 선호 순서는 공식 무료 API → 공식 파일 배포 → 라이선스 명확한 오픈소스 → 그 외 오너 판단.
🔴 그리고 **이미 이 관문을 안 거치고 쓰는 것들**을 그대로 적었습니다 — 신 네이버 JSON API(**오늘 실전이 이 위로 옮겨갔는데 약관 미확인**) · `stockanalysis.com` 의 비공식 내부 엔드포인트 · WiseReport 운영 주체 미확인 등. **이미 쓰고 있다고 확인이 면제되지 않습니다**(§0-3-12).

**테스트가 강제하는 것.** ①코드 속 모든 `http(s)://호스트` 가 목록에 있을 것 ②목록의 "코드에 있음" 주장이 참일 것 ③`requirements.txt` 전 패키지가 분류돼 있을 것 ④네트워크 성격 라이브러리 import 가 등록돼 있을 것 ⑤"요청" 호스트가 §0-3-2 본문에 있을 것. **오탐 제거**: 파이썬은 AST 문자열 상수만 보고 docstring 은 제외 — 주석·문서 속 URL 은 안 잡힙니다. **🔴 못 잡는 것도 docstring 에 적었습니다**: 새 라이브러리 뒤의 새 주소 · `"https://" + host` 처럼 쪼갠 주소 · 시크릿에서 오는 호스트 · 응답 **구조** 변화(오늘 사고 — 그건 `data_sanity`·`data_freshness` 영역). *지키는 척하는 검사가 제일 나쁩니다.*

**건강검진 워크플로우 — 설계·문서까지만, 켜지 않았습니다**(§0-3-6). 하루 **+18 요청**이라는 대가를 문서에 적어 오너가 판단하게 했습니다. 🔴 그리고 정직하게 적었습니다 — **"살아 있음"과 "우리가 쓸 수 있음"은 다릅니다.** 09-08 네이버 구 페이지는 **HTTP 200 이면서 우리한테는 0건**이었습니다. 단순 생존 확인으로는 오늘 사고를 **못 잡습니다.**

**검증.** 전체 **2,579검사 통과**(2,570 → +9). **사보타주 8종 전부 잡힘**(새 주소 추가 / 목록 행 삭제 / 가짜 패키지 / §0-3-2 에서 호스트 삭제 / 라이브러리 뒤 주소 하드코딩 / 새 라이브러리 import / 스캐너 무력화 / 블록 마커 제거). 의도적으로 통과시킨 2종(주석 속 URL · 쪼갠 주소)도 문서화했습니다.

**🔴 오너가 브라우저로 대신 확인해야 할 목록**(재고 문서 §1, 우선순위 순): ①신 네이버 API 약관 ②구 네이버 5종의 9/10 이후 운명 ③FinanceData 캐시 갱신 주기 ④`Ate329` 저장소 상태 ⑤`stockanalysis.com` 약관·robots ⑥WiseReport 운영 주체 ⑦KRX OPEN API 실응답 ⑧야후 차트 API 조건 ⑨대체재 후보 존재 확인.

**부수 발견.** `fetch_recent_volatility` 가 하루 520회 부르는 시계열이 보조지표 수집기와 **중복**입니다(§0-3-10) — 합치면 하루 약 500요청을 줄일 수 있습니다. 오너 판단 사항. 커밋 `2f54b27` + 이 커밋.

### #226 — ⛔ **이용에 조건이 달린 출처는 그 시점에 아웃** — §0-3-17 신설 (2026-09-08, 오너 지시)

**오너 지시(원문).** *"「비상업적 목적으로만 이용 가능」 이런 말이 있으면 안 하는 게 맞아. 이런 건 생각도 하지 말고 검토도 하지 마. 무슨무슨 조건이 달린 시점에서 아웃이야."*

**규칙으로 박았습니다.** `ENGINEERING_SPEC.md` **§0-3-17 신설** — 조건을 발견하는 순간 후보에서 빼고, **"그래도 쓸까요?"라고 오너에게 묻지 않습니다**(묻는 것 자체가 검토이고 오너가 그걸 하지 말라고 했습니다). 아웃 사유 예시: 비상업적·비영리 한정 / 광고 붙은 화면 사용 금지 / 수익화 금지 / 재배포·제3자 제공 금지 / 결과에 대한 대가 청구 금지 / 사전 계약·승인 필요 / 유료 라이선스로만 상업 이용. **법적에서 걸리면 비용·공식성은 보지 않습니다.** `data-sources` 에이전트의 관문 표도 같이 고쳤습니다.

**왜 이 규칙이 옳은가.** 이 서비스는 광고를 붙일 계획이 있고, 데이터를 사람에게 보여주는 것이 존재 이유입니다. 위 조건들과는 **처음부터 맞지 않습니다.** "지금은 비공개니까 괜찮다"로 붙여 두면 공개·수익화하는 날 데이터를 통째로 걷어내야 하고, 그때는 이미 그 위에 기능이 쌓여 있습니다 — §0-3-12 가 막으려는 바로 그 모양입니다.

**⛔ 즉시 아웃 판정 2건.**

**① 네이버 공식 API**(`developers.naver.com`) — 오너가 약관 전문을 확인해 주셨습니다. **7.3 ④** *"API의 결과화면과 함께 **광고를 노출하는 행위**"* 금지 · **7.3 ③** 취득 정보의 *"복제, **저장(캐시 행위 포함)**, 가공, 배포"* 금지(**우리는 스냅샷 저장·PEGY 계산·이력 누적이 본체입니다**) · **7.2 ③** 사용자에게서 *"대가(금전, 정보 제공 등 일체의 유·무형)"* 수취 금지 · **7.3 ⑥** 제3자 재제공 금지 · **8.2** *"제한된 사용권의 부여만"*.
📌 **조건 이전에 애초에 해당 서비스가 없습니다** — 네이버 공식 API 제공 목록은 검색·로그인·CLOVA·Papago·데이터랩 등이고 **증권·주식 시세 API 자체가 없습니다.** "공식 창구로 갈아탄다"는 선택지가 존재하지 않습니다.

**② KRX OPEN API**(`data-dbg.krx.co.kr`) — **약관 제6조② "비상업적 목적으로만 이용 가능, 결과에 대한 대가를 제3자에게 청구 불가."** 오너 기준으로 아웃입니다. 재고 문서의 상장종목 목록 **대체재 후보에서 제거**했습니다.

**🔴 그런데 ②는 지금 이미 쓰고 있습니다 — 오너 결정이 필요합니다.** `utils/krx_openapi.py` ← `scrape_daily.py` 가 **VKOSPI 지수값**과 **코스피200 선물 베이시스** 두 지표를 받아 `market_history.csv` 에 넣고 `/admin/macro` 화면이 씁니다. 인증키는 오너 명의로 발급(2026-08-10~2027-08-09), `KRX_OPENAPI_KEY` 환경변수 · HTTP 헤더로만 전달합니다. 이 조항은 **2026-08-10 에 이미 발견해 `macro.md` 에 적어 두었고**, 그때 판단은 *"매크로 화면이 비공개인 지금은 충돌이 없지만 공개 전환 전 재검토 필수"* 였습니다. 이번 오너 지시는 그 유예를 없애는 쪽입니다. §0-3-17 대로 **제거 여부는 오너 결정**이며, 저는 사실만 보고하고 **새로운 곳에 더 쓰는 것은 즉시 중단**합니다.

**🔴 그래서 `stock.naver.com` 은 어떤 상태인가(정직하게).** 위 네이버 약관의 **적용 대상이 아닙니다** — 클라이언트 아이디를 발급받은 적이 없기 때문입니다. 그건 "괜찮다"가 아니라 **"허락도 금지도 명시적으로 받지 않은 상태"** 이고, 구 HTML 크롤링과 법적 지위가 **같습니다**. 형식이 HTML → JSON 으로 바뀌었을 뿐 나아지지도 나빠지지도 않았습니다. 제가 "신 API" 라고 부른 것이 실제보다 공식적으로 들리게 만들었습니다 — 재고 문서에 그대로 적었습니다.

**검증.** 전체 **2,579검사 통과**.

### #227 — ⛔ 증권사 무료 API 도 아웃 + **구조가 드러남** (2026-09-08, 오너가 약관 전문 확인)

**오너 질문.** *"그러면 나 말고 다른 수많은 주식정보를 사용하고 있는 사람들 같은 경우에는 다 문제가 된다는거야?"* / *"증권사 무료 API라는걸 결국은 쓰는 수밖에 없는건가"*

**확인 결과 — 한국투자증권 KIS Developers ⛔ 아웃.** 오너가 약관 전문을 확인해 주셨습니다.
· **고객 약관 제5조③** — *"시세(국내주식, 해외주식, 국내선물/옵션 등)정보를 고객이 직접 개발한 프로그램 등 **개인의 업무에 한하여** 이용해야 하며, **제3자에게 제공해서는 아니 된다**"*
· 포털 안내 — *"시세정보는 계좌를 보유한 개인 고객이 **자기 자산의 투자 목적에 한해** 이용할 수 있으며, **제3자 제공은 불가**합니다"*
· 제휴법인 경로 — *"API를 통해 시세정보를 앱·웹 등 **화면에 표출하기 전, 코스콤 등 거래소와 별도 시세정보이용계약을 반드시 체결**해야 합니다"*
· *"제도권 금융회사(투자일임업, 투자자문업)가 아닌 **핀테크사의 경우 제휴가 불가**합니다"*

**🔴 세 곳을 전문으로 읽고 구조가 드러났습니다 — 우연이 아닙니다.** 네이버 공식 API · KRX OPEN API · KIS Developers **전부 같은 곳에서 막힙니다: "제3자에게 보여주는 것".** **시세정보의 원소유자는 거래소**이고, 증권사조차 자기가 받은 시세를 재배포할 권리가 없어서 *"너희가 직접 코스콤 등과 시세정보이용계약을 맺어라"* 라고 합니다. → **"조건 없이 무료로 남에게 시세를 보여주는" 경로는 구조적으로 존재하지 않습니다.** 특정 회사가 인색한 것이 아니라 시장 데이터 산업의 기본 구조이고, 상업적으로 하는 곳들은 전부 이 계약을 맺고 있습니다.

**그래서 오너 질문에 대한 답**: "다들 위법"이 아니라 **위치가 다릅니다.** ①개인이 자기만 보려고 쓰는 것(대부분 문제 삼지 않음) ②공개하되 무료·광고 없음(회색지대) ③공개+수익화(여기서부터 성격이 달라짐). **이 프로젝트는 ①에서 ③으로 가는 중**이라 어제까지 괜찮던 것이 문제로 보이는 것입니다 — 데이터가 바뀐 게 아니라 우리 위치가 바뀝니다.

**⚠️ 다음 세션에게.** *"무료 증권사 API 를 쓰면 되지 않나"* 를 **다시 제안하지 마세요.** 세 곳을 이미 읽었고 전부 같은 이유로 아웃입니다. 새 후보를 볼 때 **가장 먼저 볼 것은 "제3자 제공·표출이 되는가"** 이고, 거기서 걸리면 나머지는 보지 않습니다(§0-3-17).

**🔵 아직 답이 안 난 것 — 확인 가치가 있는 둘(추측 금지).** ①**종가(EOD)와 실시간 시세의 조건이 같은가.** 이 시스템은 실시간을 쓰지 않고 **장 마감 종가를 하루 한 번** 받습니다. 시장 데이터는 보통 실시간/지연/종가를 다르게 취급하는데 국내 종가 조건은 **확인 못 했습니다** → 코스콤·KRX 직접 문의가 가장 확실합니다. ②**시세(가격)와 공시(재무제표)는 출처도 성격도 다릅니다.** EPS·ROE·배당·재무제표는 **공시**에서 나오고 우리는 이미 **DART(금융감독원)** 를 씁니다. 막히는 것은 **가격** 쪽입니다. 🔴 다만 **DART 이용 조건도 아직 확인 못 했습니다** — 공공기관이라고 조건이 없다고 단정하지 않습니다.

**🔴 그리고 정직하게.** 이 사이트는 **이미 공개돼 있고 광고 계획도 있습니다.** 나중 문제가 아니라 지금 문제입니다. 저는 변호사가 아니고 위 조항들은 읽은 그대로 옮긴 것이며, **실제 판단과 대응은 오너 몫**입니다 — 사안이 사안인 만큼 전문가 확인을 권합니다. 남은 선택지는 ①코스콤·거래소와 시세정보이용계약(비용 미확인) ②수익화 포기 또는 비공개 유지 ③위 🔵 확인 결과에 따라 달라짐 — 셋 다 오너 결정입니다.

**검증.** 전체 **2,579검사 통과**.

### #228 — 🛑 **공개 서비스 중단** + 제 오진 정정 + 🔴 이관 결함 발견 (2026-09-08 밤)

**오너 결정.** *"모든 그냥 시스템 종료 / 홈페이지 폐쇄 / 비공개로 한다"* — 하루 동안 외부 데이터 출처의 이용 조건을 전부 읽은 결과(#226·#227), 시세를 제3자에게 보여주려면 거래소와 시세정보이용계약이 필요하고 코스콤 오픈API플랫폼은 법인만 가능하다는 것이 확인된 데 따른 결정입니다.

**조치.** ① **Render 웹서비스 Suspend** — 오너가 직접 실행, `visiblehand.co.kr` → *"This service has been suspended by its owner."* 확인. Delete 가 아니라 Suspend 라 설정·환경변수·도메인이 전부 남아 있고 **Resume 한 번이면 그대로 돌아옵니다.** ② **GitHub Actions 예약 실행 14개 전부 정지** — 수집 3종·결투 2종·성적표 2종·배당 3종·감시 3종·Render 깨우기. `schedule:`/`workflow_run:` 을 주석 처리하고 **`workflow_dispatch`(수동)만 남겼습니다.** 각 파일 머리말에 왜 껐는지·되살리는 법을 적었습니다. 🔴 자동 실행 정지는 **상대 서버에 계속 요청을 보내지 않기 위해서**이기도 합니다(§0-3-2 — 필요 없어진 주기 작업을 남겨두는 것도 위반).

**`utils/system_halt.py` 신설.** 멈추자 *"예약이 이 시각에 있어야 한다"* 를 지키던 테스트 **19개가 빨간불**이 됐습니다. 그 검사들은 틀린 게 아니라 지금 해당이 없을 뿐인데, 지우면 되살릴 때 무엇을 지켰는지 아무도 모르고, 방치하면 **진짜 실패를 못 봅니다**(§0-1). → `SYSTEM_HALTED` **한 줄로 선언**하고 해당 검사들이 그 값을 보고 건너뜁니다(§0-3-10). **되살릴 때 그 한 줄만 False 로** 바꾸면 전부 되살아납니다. 파일 머리말에 되살리는 4단계를 적었습니다(4번이 *"그 전에 반드시 시세정보 이용 조건이 해결됐는지 확인"* 입니다).

**🔴 제 오진 정정 — 전환 근거로 적었던 것이 틀렸습니다.** 09-08 16:11 실행에서 구 순위 페이지가 0건을 돌려준 원인을 *"Npay 증권으로 개편돼 파서가 읽던 표가 사라졌다"* 고 적었는데 **틀렸습니다.** 진짜 원인은 **FDR 404 의 연쇄** — 마스터가 ETF 만 1,167건으로 덮어써지는 바람에 구 경로의 `if ticker_types.get(code) != "STOCK": continue` 가 **모든 주식을 걸러낸** 것입니다. **구 네이버 페이지는 죽지 않았습니다** — 같은 날 19:00 실행이 같은 주소에서 전 종목 종가 **4,305건**을 정상 수집했습니다. 전환(#222) 결정 자체는 9/10 종료 예고 때문에 유효하지만, **틀린 이유를 `PROJECT_STATUS.md`·`scrape.yml`·여기 세 곳에 그대로 정정해 남겼습니다.**

**🔴 그리고 이관 결함을 발견했습니다 — 오늘 수집은 "성공"으로 끝났지만 두 항목이 통째로 비었습니다.**

| 항목 | 값 있는 종목 |
|---|---|
| `t_roe` · `t_eps` · `t_pbr` · `ev_ebitda` | 513~514 / 519 🟢 |
| `f_per` · `f_eps` | 258 🟢 (평소 수준) |
| **`f_roe`(Forward ROE)** | **0** 🔴 |
| **`div_yield`(배당수익률)** | **0** 🔴 |

**519종목 전부**에 `재무요약(Financial Summary) 표를 찾지 못했습니다` 가 붙었습니다 — #220 에서 `_fetch_ev_ebitda()` 를 `utils/wisereport_parser.py` 호출로 대체한 것과 관련된 것으로 **보이지만, 원인은 아직 확인하지 못했습니다**(EV/EBITDA 는 514종목이 정상이라 파서가 통째로 죽은 것은 아닙니다). 영향: PEGY 표본이 **200 → 175 로 12.5% 감소**(f_per·성장률 표본은 정상). `pegy_summary_history.json` 8일치로 확인했습니다.

⚠️ **산티체크는 이걸 못 잡았습니다** — 감시 대상 핵심 컬럼이 `price`·`market_cap` 뿐이라 `f_roe`·`div_yield` 는 보고 있지 않습니다. 설계상 "정상적으로 자주 비는 컬럼은 넣지 않는다"는 원칙의 결과인데, **전 종목이 0 이 되는 것까지는 못 봅니다.**

**🔴 그래서 테스트 1건을 일부러 빨간불로 남겨 뒀습니다** — `test_scoring_coverage.py::test_real_snapshot_turnaround_stocks_are_not_promoted_to_unverified`(삼성SDI). 이건 예약 정지와 무관한 **진짜 결함의 증상**이라, 건너뛰게 만들면 결함을 덮는 것입니다(§0-1). **다음에 이 저장소를 여는 사람이 가장 먼저 봐야 할 것입니다.**

**현재 테스트: 2,566 통과 / 90 건너뜀 / 🔴 1 실패(위 결함).**

**남아 있는 것(오너 판단).** ① 위 `f_roe`·`div_yield` 결함 원인 규명 ② FDR `StockListing('KRX')` 404 원인 ③ 오너 혼자 쓸 경우 **증권사 API 로 전환** — KIS 약관 제5조③ 이 허용하는 *"고객이 직접 개발한 프로그램 등 개인의 업무에 한하여 이용"* 이 정확히 그 용도이고, 크롤링보다 깔끔하며 남의 서버를 긁지 않습니다(§0-3-2).

### #229 — 📕 노선 변경을 지시서에 올림 — **중지 + 비공개 개인용 + 증권사 API** (2026-09-08, 오너 지시)

**오너 지시.** *"지금 여기까지 지시서 사항으로 올려놔줘. 전체 프로젝트는 법적인 이유로 중지로 간다고. 시스템 구성은 증권사 API를 이용하는 방향으로 비공개 혼자서 보는 용도로 방향 노선변경으로."*

**오너 지시 4번(문서 3종 분리)에 맞춰 네 곳에 나눠 적었습니다.**

| 문서 | 무엇을 적었나 |
|---|---|
| **`CLAUDE.md`** (진입점) | 맨 앞에 **🛑 중지 상태** 절 신설. *"이 항목을 읽지 않고 아무것도 시작하지 마세요."* 아웃 판정 5곳을 근거 조항과 함께 표로, 현재 상태, 노선 변경 전후 비교, 재개 시 절대 조건 3가지, 그리고 **인계 시 가장 먼저 볼 미해결 결함**까지 |
| **`ENGINEERING_SPEC.md`** (절대준수) | **§0-3-18 신설** — *"이 프로젝트는 비공개 개인용이다 — 공개·수익화는 시세정보이용계약이 선행되어야 한다."* 로그인 벽은 비공개가 아니라는 것(가입자도 제3자), 순서를 바꾸지 않는다는 것("일단 올리고 나중에"가 금지 경로), 개인 본인 용도는 KIS 약관이 **명시적으로 허용**한다는 것 |
| **`AGENT_ORCHESTRATION.md`** (전체 흐름) | 흐름도 맨 앞에 **"이 흐름도는 멈춰 있는 시스템을 그린 것"** 경고 + 옛 노선/새 노선 대비 그림. 🟢 **버려지는 것은 "출처"와 "공개 배포"뿐** — 수집기 골격·3단계 검증·산티체크·신선도 감시·출처 재고 체계는 **출처만 갈아끼우면 그대로** |
| **`PROJECT_STATUS.md`** (현재 현황) | 맨 앞에 **지금 상태** 절 — 멈춘 것 6줄(되돌리는 법 포함) · 왜 멈췄나 · 앞으로 갈 방향 · 재사용/폐기 목록 · 🔴 미해결 결함 · 그 밖에 남은 것(FDR 404, 전 종목 종가 구 주소, KRX OPEN API 제거 여부, 도메인 자동갱신) |

**노선 요약.** 공개 서비스 + 크롤링 → **비공개 개인용 + 증권사 공식 API.** KIS 약관 제5조③ 이 금지한 것은 "제3자 제공"이고 *"고객이 직접 개발한 프로그램 등 **개인의 업무에 한하여** 이용"* 은 명시적으로 허용합니다 — 새 노선이 정확히 그 용도이며, 부수적으로 §0-3-2(남의 서버를 긁지 않음)도 같이 해결됩니다.

**🔴 테스트 1건은 계속 빨간불입니다** — `test_real_snapshot_turnaround_stocks_are_not_promoted_to_unverified`. 예약 정지와 무관한 **진짜 결함의 증상**(#228 의 `f_roe`·`div_yield` 전 종목 결측)이라 건너뛰게 만들지 않았습니다(§0-1). 이제 진입점·현황 문서 **양쪽에서 이 결함을 가리킵니다.**

**현재 테스트: 2,561 통과 / 95 건너뜀 / 🔴 1 실패(위 결함).**

### #230 — ⛔ 금융위 API 제4유형 전환 + **유료 경로 추진 안 함(오너 결정)** (2026-09-09)

**들어온 것.** 제공기관(금융위원회) 통보 메일 — **OPEN API 10종**(주식시세·지수시세·KRX상장종목정보 등, `apis.data.go.kr/1160100/...`)의 이용허락범위가 **제1유형(출처표시) → 제4유형(출처표시 + 상업적이용금지 + 변경금지)** 으로 변경. **기존 V1 을 계속 써도 일괄 적용**되고, 기존 URL 은 90일 후 중지. 안내문: *"본 데이터는 **상업적 목적 여부와 상관 없이 제3자 무단 제공 및 재배포가 엄격히 금지**됩니다."* → §0-3-17 대로 **아웃**, 재고 문서에 근거와 함께 기록했습니다.

**그 메일에 처음 나온 것.** 어제까지 확인한 문서는 전부 *"안 된다"* 였는데, 이 메일은 **"유료로 사면 무엇이 되는지"** 를 처음으로 적어 뒀습니다 — *"KRX Data Marketplace 에서 유료로 구매… 이 경우에도 ①가공을 통해 독창성 있는 저작물을 생산 ②제3자의 방문, 가입을 통한 간접 수익 창출 목적으로만 상업적 활용이 가능"*(문의: 한국거래소 데이터사업부 `krxdata@krx.co.kr`). 공교롭게도 이 프로젝트의 성격(원시 시세가 아니라 PEGY·그레이엄 넘버 등 **가공 지표**, 광고는 **간접 수익**)과 겹치는 조건이라 그대로 보고했습니다.

**🔴 오너 결정 — 추진하지 않습니다.** *"안 해. 물어보지 않을 거야. 그쪽으로 더 이상 연결을 하려고 안 해도 돼."*

**⚠️ 이 기록의 목적은 다음 세션이 이걸 다시 파지 않게 하는 것입니다.** 문서에 *"비용 미확인 — 문의 필요"* 로 남겨 두면 다음에 여는 사람이 **처음부터 다시 조사합니다.** 그래서 `CLAUDE.md`(진입점)와 `DATA_SOURCES_INVENTORY.md` 양쪽에 **"미확인이 아니라 결정"** 이라고 명시하고, *비용 문의·개인 구매 가능 여부·조건 해당성을 다시 제안하지 말 것* 을 못 박았습니다. **조사할 것이 남아서 멈춘 게 아니라, 하지 않기로 정해서 멈춘 것입니다.**

→ 이 프로젝트는 `ENGINEERING_SPEC.md` §0-3-18 대로 **비공개 개인용**으로만 갑니다.
