...
- 🔴 **#175 SPEC §5-1 2중 Cap 코스피 미구현 — 오너 결정 필요.** `collector_kospi200.py`의
  `g_eff` 계산에 성장률 35%·주주환원 10%·총합 40% 캡이 전부 빠져 있음(미국 경로엔
  있음). 200종목 중 17종목 PEGY 밴드가 바뀜. ① 코드를 명세대로 고칠지 ② 명세를
  코드에 맞춰 고칠지 오너 확인 후 진행.
- 🆕 #175 `format_amount()` NaN/무한대 방어 누락(현재 실데이터로는 재현 안 됨,
  `_positive_number()`처럼 `math.isfinite()` 가드 추가 검토).
- 🆕 #175 `ENGINEERING_SPEC.md` §5-4 표가 2026-08-06 개편 이후 갱신 안 됨(PER 임계값·
  고배당 DPS=0 처리 등 3행이 실제 코드와 다름) — 문서만 최신화하면 됨.
- 🆕 #175 역성장 종목(`g_eff<=0`)은 배당 미수집 배지를 못 받음(`guardrail.py` 조기
  return) — 현재 실데이터로 두 조건이 안 겹쳐 안 보이지만 구조적 갭.
- 🆕 #175 `utils/data_validator.py` 커버리지 50% — PEGY 카드 차단 여부를 결정하는
  3단계 검증 하네스인데 커버리지가 가장 낮음. 다음 커버리지 보강 1순위 후보.
