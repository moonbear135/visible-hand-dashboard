...
표이고, 가격 파일은 저장소에 커밋돼 있는 수집 결과입니다. 즉 선행 배치가 없습니다.
  ⚠️ 다만 **가격 수집(`scrape.yml` / `scrape_us.yml`)이 끝난 뒤**에 도는 편이 낫습니다 —
     ...
     사용자들은 (0% 로 채워지는 것이 아니라) 그날 발행에서 빠집니다. 또한 #201 의 신선도
     검사가 미국 벤치마크(`scrape_report_snapshots.yml` → `data/us_index_history.json`,
     08:20 KST)를 읽으므로 그 뒤여야 미국 쪽 검사가 살아 있습니다(#202 에서 cron 을
     11:35 KST 로 옮긴 이유). 워크플로우 파일의 cron 주석에 그 시각 계산을 적어 뒀습니다.
