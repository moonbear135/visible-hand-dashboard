...
#    2026-08-20 추가 (동의 화면 · 공개 순위표 화면)
# =============================================================================
#  위 두 값(`DUEL_ENABLED` / `DUEL_MENU_ADMIN_ONLY`)은 **1갈래 "덤벼라 나 자신"** 전용이고,
#  아래 네 값은 **2갈래(공개)** 전용입니다. 헷갈리지 않게 이름에 화면을 그대로 박았습니다.
#
#  🔴 왜 1갈래 스위치를 재사용하지 않았나 (오너 확정 방침을 코드로 옮긴 것):
#     · 오너 방침(2026-08-20): *"일정 사람이 쌓이기 전까지 공개를 하지 않는 방향으로 가자,
#       만들 수 있을 때는 만들어만 두는 건 좋을 거 같아."* 그리고 작업지시서 7-3: *"1갈래만
#       먼저 공개해도 됩니다 … 1갈래 단독 배포 → 사용자 모집 → 그 뒤 2갈래 공개가 자연스러운
#       순서입니다."* 스위치가 하나뿐이면 이 순서 자체가 불가능합니다.
#     · 두 2갈래 화면도 서로 시점이 다릅니다. 동의 화면은 "참가자를 모으는" 화면이라 먼저
#       열어야 하고, 순위표 화면은 최소 인원(5-6)이 찰 때까지 어차피 아무것도 못 보여줍니다.
#       그래서 화면마다 한 쌍씩 둡니다.
#
#  🔴 **의존 관계**: 2갈래 화면은 `DUEL_ENABLED` 가 켜져 있을 때만 존재합니다(아래 메뉴
#     조립과 두 화면 파일 양쪽에서 `DUEL_ENABLED and ...` 로 확인합니다). 1갈래가 숨겨져
#     있으면 계좌 자체가 없고, 계좌가 없으면 공개할 성적도 없습니다.
#
#  판정 관례는 위와 같습니다 — 환경변수 값이 **정확히 "true"(대소문자 무관)** 일 때만 켜집니다.

#: 🔓 공개 동의 관리 화면(`/duel/consent`, 작업지시서 5-2 · 5-8) 기본 숨김 스위치.
...
DUEL_LEADERBOARD_ENABLED = (
    (os.environ.get("DUEL_LEADERBOARD_ENABLED") or "").strip().lower() == "true"
)
...
#    2갈래(공개) 화면 두 개도 **같은 그룹 안에** 넣되, 각자의 스위치가 켜졌을 때만
#    항목이 생깁니다. `DUEL_ENABLED` 블록 **안에** 둔 것이 의존 관계 그 자체입니다 —
#    1갈래가 숨겨진 상태에서 공개 화면만 켜지는 조합을 만들 수 없습니다.
#    ⚠️ 항목 목록을 `_duel_items = [...]` 같은 **이름 붙은 전역**으로 만들지 않았습니다 —
#       `tests/test_web_session_isolation.py` [1-a] 가 "모듈 최상위 가변 전역"을 전부 잡아
#       사유 등록을 요구하기 때문입니다(§0-3-8). 여기서는 목록을 곧바로 `_MENU_GROUPS` 에
#       넣어 새 전역 이름을 만들지 않습니다.
if DUEL_ENABLED:
    _MENU_GROUPS.insert(-1, ('⚔️ 결투다!',
        [('/duel', '⚔️ 참전하기', DUEL_MENU_ADMIN_ONLY)]
        + ([('/duel/consent', '🔓 공개 동의 관리', DUEL_CONSENT_MENU_ADMIN_ONLY)]
           if DUEL_CONSENT_ENABLED else [])
        + ([('/duel/leaderboard', '🏆 공개 순위표 (내 밑으로 눈 깔어)',
             DUEL_LEADERBOARD_MENU_ADMIN_ONLY)]
           if DUEL_LEADERBOARD_ENABLED else [])
    ))
