...
#     (`duel_rules.MIN_PARTICIPANTS_FOR_PUBLICATION` — 당시 500명, 2026-09-03 부터 1명)이
#     찰 때까지 어차피 아무것도 보여줄 수 없습니다.
#
...
# 인원(당시 500명 — 2026-09-03 에 1명으로 낮춤) 문턱에 못 미쳐 "참가자가 부족합니다" 안내만 뜨는 상태라도 먼저 열어서 사람들
...
#
# ⏳ 2026-08-24 — **2단계(관리자 전용)에서 시작했습니다.** 결투(2026-08-20)와 성적표 공개
# 계층(2026-08-23)이 밟은 것과 같은 순서입니다. 이 모듈은 데이터 자체에 오너가 확인해야
# 할 한계가 있었습니다(수집 대상이 "전 상장사"가 아니라 2023~2025년 배당 이력 종목뿐 —
# `DIVIDEND_MODULE_WORK_ORDER.md` [항목 1]). 그 사실은 화면 상단에 그대로 적혀 있습니다.
#
# ✅ 오너 확정 (2026-08-25) — **3단계(전체 공개)로 전환합니다.** 8월 백필 실행(91건 확보,
# 롯데케미칼 등 실제 데이터 확인)·리츠 파싱 구멍 발견 및 수정·배당락일 계산 기능까지
# 오너가 실 데이터로 직접 화면을 확인한 뒤 내린 결정입니다. 이전엔 여기서 `True` 를
# 확인했었습니다 — 이력은 지우지 않고 값만 뒤집어 남겨둡니다(위 결투·성적표 스위치와
# 같은 관례, §0-3-10).
...
#: 2단계(관리자 전용) ↔ 3단계(전체 공개)를 가르는 **단 하나의 불리언**.
#: ⏳ 2026-08-27 — 2단계(관리자 전용)에서 시작. 위 39%/61% 비대칭을 오너가 실 데이터로
#:    직접 확인한 뒤 공개 여부를 정합니다(한국 배당·보조지표 모듈이 밟은 것과 같은 순서).
#
#: ✅ 오너 확정 (2026-08-27, 같은 날 저녁) — **3단계(전체 공개)로 전환합니다.** 실 배포
#: 화면(관리자 계정)으로 사이드바 라벨·달력·매수 마지막 날 계산값·145종목 "배당 없음
#: 확인됨" 구획까지 직접 확인한 뒤 "된 것 같다 공개로 돌려도 될 것 같아"로 확정. 이전엔
#: 여기서 `True`를 확인했었습니다 — 이력은 지우지 않고 값만 뒤집어 남겨둡니다(한국
#: 배당·보조지표 스위치와 같은 관례, §0-3-10).
DIVIDEND_US_MENU_ADMIN_ONLY = False
...
#
# 🔁 2026-08-26 (오너 요청) — 상위 그룹 이름 3개를 더 직관적으로 바꿨습니다. 화면
#    파일·라우트·하위 항목 라벨은 전부 그대로이고, 사이드바 상위섹터명만 바뀝니다
#    (그룹명만 바꾼 '💰 배당 달력'(2026-08-25)과 같은 방식 — §0-3-10).
#      '💡 사실 이 가격이에요' → '💡 재무제표 읽기' (오너가 "제무재표"로 적었으나
#         맞춤법상 "재무제표"가 맞아 그대로 반영)
#      '📊 보유종목'          → '📊 내 수익률 관리'
...
_MENU_GROUPS = [
    ...
    #    **여기 없습니다** — 아래 `⚔️ 수익률 비교` 그룹으로 옮겼습니다(2026-08-23,
...
# ⚔️ 수익률 비교 (구 "⚔️ 내 밑으로 눈 깔어", 2026-08-26 개명 · 그 전엔 "⚔️ 결투다!",
#    2026-08-23 개명) — "결투다!"(가상계좌 연습) 화면과
...
if DUEL_ENABLED or SCORECARD_CONSENT_ENABLED or SCORECARD_LEADERBOARD_ENABLED:
    _MENU_GROUPS.insert(-1, ('⚔️ 수익률 비교', (
...
# 💰 배당 달력 (배당금 모듈, 2026-08-24 추가) — 위 결투 그룹과 **같은 자리·같은 방식**입니다.
#    `insert(-1)` = 마지막 '⚙️ 관리자' 그룹 바로 앞(관리자 그룹은 항상 맨 끝). 앞으로 이 모듈에
#    화면이 늘어나면 여기 한 줄씩 붙습니다(결투 그룹이 그렇게 자랐습니다). 꺼져 있으면 숨긴
#    링크를 CSS 로 가리는 게 아니라 **메뉴 데이터에 아예 존재하지 않습니다**(§0-3-6).
#    🔁 2026-08-25 (오너 요청) — 그룹 이름(상위섹터명)만 '💰 투자 감사합니다!' → '💰 배당 달력'
#       으로 바꿨습니다. 애초엔 항목이 하나뿐이라 그룹 이름과 항목 라벨이 같았는데, 오너가
#       보기에 상위섹터 이름과 화면 안내문이 똑같이 반복되는 게 어색해 보여 그룹 이름만
#       구분되게 바꿔 달라고 했습니다. 항목 라벨('/dividend' 링크에 뜨는 글자, 화면 헤더 우측에
...
#
#    ⚠️ 오너 요청(2026-08-25) — 사이드바 **그룹 이름**만 "보조지표 해석"으로 바꿈. 화면
#       자체의 제목(`/indicator` 페이지 안의 "🙏 여기서부터는 신앙입니다")과 링크 라벨은
#       그대로 둠 — 그룹명은 메뉴에서 한눈에 무슨 기능인지 보이게, 화면 제목은 이 모듈의
#       정체성(§1 확정 이름)을 유지하는 자리라 서로 역할이 다름.
if INDICATOR_ENABLED:
    _MENU_GROUPS.insert(-1, ('📐 보조지표 해석', [
