...
⚠️ §0-1 — 캐시 키에 **파일 수정시각(mtime)과 크기**를 넣습니다. 그래야 수집기가 새 스냅샷을
   덮어썼을 때 옛 데이터를 계속 보여주는 사고가 나지 않습니다. 로드에 실패하면 조용히
   이전 값을 돌려주지 않고 실패를 그대로 알립니다(호출한 화면이 빨간 배너를 띄웁니다).
...
def read_download_bytes(path: str) -> Optional[bytes]:
    ...
    try:
        ...
        print(f'⚠️ 다운로드용 파일 읽기 실패 ({path}): {exc}')
        return None
    except UnicodeDecodeError as exc:
        print(f'⚠️ 다운로드용 파일 디코딩 실패 ({path}): {exc}')
        return None
