...
/* 2026-08-21 (오너 지적: "규칙끼리 구분이 안 된다") — 결투 화면의 "규칙 전체 보기"
   접이식 설명(web/pages/duel_page.py 의 `_render_rules_expansion*`)에서 규칙 N 과
   규칙 N+1 사이 간격이 한 규칙 안의 문단 간격과 똑같아, 어디서 규칙이 끝나고 다음
   규칙이 시작되는지 눈으로 구분되지 않았습니다.
   ⚠️ 빈 줄을 더 넣는 방식(`\\n\\n\\n\\n`)으로는 고쳐지지 않습니다 — 마크다운은 연속된
      빈 줄을 문단 나눔 하나로 접어버려서 렌더 결과가 `\\n\\n` 과 완전히 같습니다.
      그래서 규칙 사이에 실제 구분선(`---` → `<hr>`)을 넣고, 그 `<hr>` 의 위아래
      여백(28px)을 여기서 문단 간격보다 확실히 넓게 줍니다.
   색은 이 파일이 이미 옅은 경계선에 쓰는 값(`.vh-card` 와 동일)을 그대로 씁니다. */
.vh-rule-divider hr {
    margin: 28px 0;
    border: none;
