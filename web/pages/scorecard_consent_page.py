...
② **여섯 번째 독립 동의(`consent_real_principal_bracket`)가 없습니다.** 결투에서 그 동의가
   따로 있던 이유는 "다른 모듈('내 성적표')의 실제 자산 데이터를 끌어다 쓴다"는 것이었는데,
   여기서는 **공개되는 데이터 자체가 이미 그 실제 자산**입니다. 종목별 매입금액을 공개하기로
   한 순간 체급의 입력값(매입원가합계)은 이미 공개된 값들의 단순 합이라, 두 번째 동의
   게이트를 세울 대상이 남아 있지 않습니다(`utils/scorecard_publish.py` 머리말 ③ ·
   `utils/scorecard_publish_db.CONSENT_ITEM_FLAGS` 주석 · 스키마 §2-2 와 같은 판단).
   🔴 그러므로 이 화면에 여섯 번째 체크박스를 **만들지 마세요.** 만드는 순간 화면이 저장
      하려는 항목과 DB 의 `scorecard_consent_final_requires_all` CHECK 가 갈라집니다.
...
📝 문구에 대하여
-------------------------------------------------------------------------------
· 항목별 동의 5문장은 결투 문안을 **그대로 옮긴 것이 아니라 다시 쓴 것**입니다 — 공개
...
#  🔴 다섯 문장 모두 **실제 보유 자산**을 말합니다. "가상계좌"·"결투"라는 말이 이 화면에
...
NOTICE_REAL_DATA = (
    ...
    "보유 자산입니다.\n\n"
    "종목·수량·매입금액이 그대로 다른 이용자에게 보입니다. 기본값은 비공개이고, 동의하지 "
    "않으시면 공개 순위표 어디에도 나타나지 않습니다."
)
...
#:  위 5개 항목으로 이미 공개하기로 한 값들의 합입니다. 그래서 이 문구는 "왜 여섯 번째 동의
#:  항목이 없는가"를 함께 설명합니다 — 없는 항목을 그냥 안 그리고 넘어가면, 결투 화면을 본
#:  적 있는 사용자는 "체급 동의를 안 했는데 왜 체급이 정해졌지"라고 생각하게 됩니다(§0-1).
...
def consent_item_rows():
    """
    화면에 그릴 항목별 동의 5개를 `[(flag, 이름, 문장), ...]` 로.
    ...
    🔴 여기서 `scorecard_publish_db.CONSENT_ITEM_FLAGS` 와 **키·순서가 같은지** 확인합니다.
       ...
       이 확인 하나가 "여섯 번째 항목을 슬쩍 끼워 넣는" 변경도 함께 막습니다.
    """
...
def all_items_checked(values):
    """5개가 **전부** 체크됐는가('전부 아니면 전무'의 화면 쪽 판정)."""
...
def final_confirm_payload(values):
    ...
    5개를 다시 넘기는 이유: `scorecard_publish_db.save_consent()` 는 **이번에 보낸 payload
    안에서** 5개가 전부 켜져 있는지를 봅니다(DB CHECK 와 같은 규칙을 앱에서 한 번 더 보는
...
async def _render_consent_card(client, user_id: str, on_changed) -> None:
    """동의 카드 — 현재 상태 → 1단계(항목별 5개) → 2단계(최종 확인) → 철회."""
...
def _render_consent_form(client, user_id: str, state: dict, on_changed) -> None:
    ...
    final_box = ui.checkbox(
        '위 5개 항목 전부를 읽고 이해했으며, 내 성적표의 실제 보유 내역을 공개 순위표에 '
        '공개하는 데 최종적으로 동의합니다.'
    ).props('dense').classes('w-full vh-keep-all')
