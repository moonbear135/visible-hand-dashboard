...
NOTICE_NO_DIVIDEND_USD = (
    "배당금은 반영되지 않습니다. 미국 주식 수집기에는 배당락일 한 개만 '수집 시점에 화면에 "
    "떠 있던 값'으로 들어와 있을 뿐, 사건별 지급일과 확정 주당 금액을 담은 배당 지급 "
    "캘린더가 없습니다. 추정치로 배당을 흉내 내는 대신 아예 반영하지 않는 쪽을 택했습니다."
)
...
NOTICE_BUY_ONLY_USD = (
    "달러 트랙도 매수만 가능합니다. 매도는 지원하지 않습니다 — 한 번 산 종목은 팔 수 없고, "
...
NOTICE_NO_FX_MIX = (
    "원화 계좌와 달러 계좌의 숫자는 어디서도 더하지 않습니다. 이 앱에는 환율 시계열이 "
    "없고, 환율을 지어내 두 통화를 하나의 '총자산'으로 합치는 순간 그 숫자는 사실이 "
    "아니게 됩니다. 시드머니도, 정기입금도, 수익률도, 순위표도 두 트랙이 완전히 별개입니다."
)
...
NOTICE_CRAWL_FAILURE_USD = (
    ...
    "다음 날로 미뤄지지 않고 그 자리에서 취소됩니다. 취소된 주문에는 사유가 남고 예수금은 "
...
NOTICE_CASH_ROLLOVER_USD = (
    "쓰지 않은 예수금은 사라지지 않습니다. 다음 달로 그대로 넘어가고, 억지로 어딘가에 "
    "투자되지도 않습니다. 예수금이 남아 있는 한 언제든 매수할 수 있고, $0이 되면 다음 "
    "입금일까지 매수만 잠깐 쉬게 됩니다(별도의 '매수 기간' 제한은 없습니다)."
)

NOTICE_TWR_USD = (
    "달러 계좌의 '누적 수익률'도 시간가중수익률(TWR)입니다 — 계산식은 원화 계좌와 완전히 "
    "같은 함수를 씁니다(입금이 있던 날은 그 입금액을 빼고 계산). 다만 계산에 들어가는 "
