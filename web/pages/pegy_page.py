"""
💡 사실 이 가격이에요 — 코스피+코스닥 밸류에이션 리포트 (공개 화면, URL `/kr`).

🔁 2026-09-04 — URL 이 `/` → `/kr` 로 옮겨졌습니다(구글 애드센스 심사 대응). NiceGUI 화면은
   서버가 내려주는 최초 HTML 에 본문 텍스트가 전혀 없고(웹소켓 연결 뒤 브라우저가 그림),
   그래서 크롤러 눈에는 `/` 가 빈 페이지였습니다("가치가 별로 없는 콘텐츠" 반려 — 실측 확인).
   지금 `/` 는 `web/pages/landing_page.py` 의 **순수 FastAPI 정적 소개 페이지**가 맡고, 이
   화면은 미국판 `/us` 와 짝을 이루는 `/kr` 에서 그대로 서비스됩니다. 화면 내용은 한 글자도
   바뀌지 않았습니다 — 바뀐 것은 데코레이터의 경로 문자열 하나뿐입니다.
...
def load_latest_kospi_usd(csv_path=None):
    ...
    ⚠️ **동기 함수입니다 — 화면에서 직접 부르지 마세요.** 부르는 자리
       ...
       느려지고 무엇보다 **이 화면(당시 '/', 지금 '/kr')이 이번 사고가 실제로 신고된 화면**입니다. 한
