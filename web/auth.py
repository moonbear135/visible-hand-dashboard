...
#  └────────────────────────────────────────────────────────────────────────┘
#
# ⚠️ `utils/scorecard_db.create_supabase_client()` 의 경고 주석("이 클라이언트를 프로세스
#    전역·접속자 단위로 캐시하면 로그인 세션이 모든 방문자에게 공유된다" — 2026-08-29
#    재감사 전에는 Streamlit 의 `@st.cache_resource` 를 가리켰습니다)이 NiceGUI 에서 그대로,
#    오히려 더 쉽게 재현됩니다. 이 파일에서 클라이언트를 만드는 곳은 `get_client()` 한
#    군데뿐이고, 그 결과는 항상 `client_storage()` 에만 들어갑니다.
# =============================================================================
