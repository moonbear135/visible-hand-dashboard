...
def render_study_only_section():
    ...
    with st.expander("📚 공부용 참고 — 지금은 다루지 않는 지표 (직접 공부해보고 싶다면)"):
        st.info(
            "ℹ️ **아래 지표들은 이 화면의 점수에서 다루지 않고 있습니다.** "
            "예전에는 점수에 들어가 있었지만, 실제로는 실측 데이터 없이 '코스피 등락률·환율·수급 부호·변동성'을 "
            "그럴듯한 이름으로 바꿔 부르는 수준이어서 2026-08-10에 빼냈습니다.\n\n"
            "**빼낸 이유는 두 종류입니다.** ① 앞의 4개는 무료로 구할 수 있는 데이터 자체가 없습니다. "
            "② 마지막 2개(공매도 거래 비중 · 공매도 잔고)는 **데이터는 존재하지만**, 그것을 받아올 수 있는 "
            "유일한 무료 경로가 로그인 우회 방식이라 이 프로젝트의 크롤링 원칙상 쓰지 않기로 했습니다. "
            "각 항목의 '② 지금 왜 못 다루나요?'에 어느 쪽인지 그대로 적어두었습니다.\n\n"
...
def render_macro_page():
    ...
    st.warning(
        ...
        "위험 지표 **6개 중 2개**(외환 스왑포인트 · 풋옵션 미결제약정)가 아직 실데이터가 아닌 추정 프록시 "
        "공식(코스피·환율·수급 값으로 계산)에 의존하고 있어, ENGINEERING_SPEC.md §0-3-1 원칙(후행지표 전용)에 "
        "맞게 재설계될 때까지 비공개 상태로 둡니다. "
        "(2026-08-10 기준 **KOSPI 5일 수익률 · 주식 현물 순매도**(#68)와 **VKOSPI · 선물 베이시스**(#70, KRX "
        "OPEN API) **4개는 실측 전환 완료**, 실측 경로가 없던 6개(#69)와 공매도 2개(#72)는 점수에서 제외 "
        "— 아래 '📚 공부용 참고' 섹션 참조)"
    )
    ...
    with st.expander(f"🔍 {n_indicators}개 변동성 지표별 위험 기여도 상세 분석표 보기"):
        ...
        st.info(
            f"ℹ️ **지표 산출 방식 안내** — 아래 {n_indicators}개 중 **2개**(외환 스왑포인트 · 풋옵션 미결제약정)는 "
            "실제 파생상품 시장에서 직접 수집한 값이 아니라, **KOSPI 종가 · 원/달러 환율 · 두 값의 전일 대비 "
            "변화율 · 투자자 3주체 수급** 실측값으로부터 위 '산출 공식'에 따라 계산한 **추정 프록시(대용치)** 입니다. "
            "**'실측' 표기가 붙은 4개**(KOSPI 5일 수익률 · 주식 현물 순매도 · VKOSPI · 선물 베이시스)는 "
            "실제 측정값을 과거 분포 대비 정규화한 값입니다. "
            "'데이터 없음'으로 표시된 지표는 입력값이 없어 산출하지 못한 것이며, 종합 점수의 가중평균에서도 제외됩니다. "
            "\n\n2026-08-10부터, 실측 경로가 아예 없어 '이름만 파생상품'이던 6개 지표와, 공매도 2개 지표"
    ...
    with st.expander("📝 공식 알고리즘 거버넌스 및 5W1H 변경 이력 (Audit Trail)"):
        ...
        tab_72, tab_69, tab_new, tab0, tab1, tab2, tab3 = st.tabs(
            ["📄 v1.7.0 (공매도 2종 제외·가중치 재분배)", "📄 v1.6.0 (실측불가 6개 제외·가중치 재분배)", "📄 v1.5.0 (2차 감사·가중치 단일화)", "📄 v1.4.0 (데이터 무결성 감사 반영)", "📄 v1.3.1 (로직 원복)", "📄 v1.2.0 (더미 제거 시도)", "📄 v1.0.0 (최초 배포)"]
        )
