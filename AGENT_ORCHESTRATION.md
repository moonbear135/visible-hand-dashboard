...
## 🛑 2026-09-08 — 이 흐름도는 **멈춰 있는 시스템**을 그린 것입니다

아래 파이프라인(크롤링 → 검증 → 화면 → 배치)은 **2026-09-08 오너 결정으로 전부 정지**했습니다.
법적 근거와 경위는 `CLAUDE.md` 맨 앞, 규칙은 `ENGINEERING_SPEC.md` §0-3-17·§0-3-18.

**노선이 바뀌었습니다:**

```
  [옛 노선 — 중지]                        [새 노선 — 앞으로]
  네이버 웹/JSON 크롤링                     증권사 공식 API (계좌 인증)
        ↓                                        ↓
  수집 → 검증 → 저장                        수집 → 검증 → 저장   ← 이 부분은 그대로 재사용
        ↓                                        ↓
  Render 공개 웹 + 광고 계획      →→→→      🔴 비공개 · 오너 본인만
  (제3자 제공 → 계약 필요)                  (KIS 약관이 허용하는 용도)
```

🟢 **버려지는 것은 "출처"와 "공개 배포"뿐입니다.** 수집기 골격·3단계 검증·산티체크·
신선도 감시·데이터 출처 재고 체계는 **출처만 갈아끼우면 그대로 돕니다.**

⛔ **다시 켤 때 지킬 것**: 공개하지 않습니다(로그인 벽도 비공개가 아닙니다). 새 출처는
§0-3-17(조건 달리면 아웃)을 먼저 통과해야 합니다. 절차는 `utils/system_halt.py` 머리말.
