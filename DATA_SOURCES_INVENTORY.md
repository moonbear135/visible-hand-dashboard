...
| **금융위원회 OPEN API 10종**(`apis.data.go.kr/1160100/...` — 주식시세·KRX상장종목정보 등) | ⛔ **2026-09-09 통보로 이용허락범위가 제1유형(출처표시) → 제4유형(출처표시 + **상업적이용금지** + **변경금지**)으로 변경.** 기존 V1 을 계속 써도 **일괄 적용**됩니다. 안내문: *"본 데이터는 **상업적 목적 여부와 상관 없이 제3자 무단 제공 및 재배포가 엄격히 금지**됩니다"* | 2026-09-09 (제공기관 메일) |
| **코스콤 오픈API플랫폼**(BaaS) | ⛔ **이용 대상 자체가 안 됩니다** — *"오픈API플랫폼은 **「법인」기업만 이용 가능**합니다. **개인사업자, 일반인, 학생 등은 이용할 수 없습니다**"* · *"핀테크기업[법인;스타트업]이 아닌 개인사업자, 일반인, 학생 등은 오픈API플랫폼 서비스 이용이 불가"* | 2026-09-08 (오너 확인, `koscom.gitbook.io/open-api/how-to-use/procedure/charge`) |
...
### ⛔ 유료 구매 경로 — **오너가 추진하지 않기로 결정했습니다** (2026-09-09)

2026-09-09 금융위 통보 메일에 **처음으로 "유료로 사면 무엇이 되는지"** 가 적혀 있었습니다:

> *"상업적 목적으로 이용할 경우 원천 소유자인 한국거래소의 **KRX Data Marketplace**(data.krx.co.kr)
> 에서 **유료로 구매**하여야 합니다. 이 경우에도 아래의 목적으로만 상업적 활용이 가능합니다.
> 1. 가공을 통해 독창성 있는 저작물을 생산  2. 제3자의 방문, 가입을 통한 간접 수익 창출"*
> 문의: 한국거래소 데이터사업부 `krxdata@krx.co.kr`

🔴 **그런데 오너가 이 경로를 추진하지 않기로 했습니다** — *"안 해. 물어보지 않을 거야.
그쪽으로 더 이상 연결을 하려고 안 해도 돼."*

⚠️ **다음 세션에게 — 이건 "미확인"이 아니라 "결정"입니다.**
비용을 알아보자거나, 개인 구매가 되는지 문의해 보자거나, 위 두 조건에 우리가 해당하는지
따져 보자는 제안을 **다시 꺼내지 마세요.** 오너가 이미 판단했습니다. 조사할 것이 남아
있어서 멈춘 게 아니라, **하지 않기로 정해서** 멈춘 것입니다.

→ 이 프로젝트는 `ENGINEERING_SPEC.md` §0-3-18 대로 **비공개 개인용**으로만 갑니다.

### 🔵 참고로 남겨두는 것 (추진하지 않음 — 위 결정 참고)

1. **🟢 종가(EOD)는 실시간과 별개 계약 항목인 것이 확인됐습니다**(2026-09-08, 코스콤 페이지).
   코스콤이 계약서 양식을 **두 개로 나눠** 올려두고 있습니다:
   · `스타트업-[KRX] **종가정보** 이용계약_20180711.hwp`
   · `스타트업-[KRX] **실시간정보** 이용계약_20180711.hwp`
   → 우리는 **실시간을 쓰지 않고 장 마감 종가만** 하루 한 번 받으므로, 따져야 할 것은
   **종가정보 이용계약** 쪽입니다. 🔴 다만 **그 계약의 내용·비용·개인(법인 아님) 체결 가능
   여부는 확인하지 못했습니다.** 계약서 파일이 `.hwp` 라 이 세션에서 열 수 없습니다.
   → **문의처(코스콤 시장데이터영업팀): `fintechdata@koscom.co.kr` · 02-767-7537.**
   ⚠️ 위 "스타트업 지원"은 **스타트업(법인)** 대상으로 보입니다 — 개인이 종가정보 이용계약을
   맺을 수 있는지는 **문의해야 알 수 있습니다.** 추측하지 않습니다.
